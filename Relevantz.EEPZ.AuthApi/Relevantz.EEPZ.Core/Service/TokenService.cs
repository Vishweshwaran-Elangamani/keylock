...
//         {
//             var issuer = _configuration[TokenConstants.ConfigKeys.JwtIssuer]
//                 ?? TokenConstants.Defaults.DefaultIssuer;
//             var audience = _configuration[TokenConstants.ConfigKeys.JwtAudience]
//                 ?? TokenConstants.Defaults.DefaultAudience;
//             var secretKey = _configuration[TokenConstants.ConfigKeys.JwtSecretKey]
...
//             var empId = _context.Employeedetailsmasters
//                 .Where(edm => edm.EmployeeId == user.EmployeeId)
//                 .Select(edm => edm.EmployeeId)
//                 .FirstOrDefault();

//             var empMasterId = _context.Employeedetailsmasters
//                 .Where(edm => edm.EmployeeId == user.EmployeeId)
...
//         public async Task<string> GenerateAccessTokenFromEmployeeAsync(Employee employee)
//         {
//             var issuer = _configuration[TokenConstants.ConfigKeys.JwtIssuer]
//                 ?? TokenConstants.Defaults.DefaultIssuer;
//             var audience = _configuration[TokenConstants.ConfigKeys.JwtAudience]
...
//                 .Where(edm => edm.EmployeeId == employee.EmployeeId)
//                 .FirstOrDefaultAsync();

//             var role = await _context.Roles
//                 .Where(r => r.RoleId == (empMaster != null ? empMaster.RoleId : 0))
//                 .FirstOrDefaultAsync();

//             // ✅ FIXED: Use RoleName (Role entity has RoleName not Name)
//             var roleName = role?.RoleName ?? "Employee";

//             var userAuth = await _context.Userauthentications
//                 .Where(ua => ua.EmployeeId == employee.EmployeeId)
//                 .FirstOrDefaultAsync();

//             var userId = userAuth?.UserId ?? 0;

//             return JwtHelper.GenerateAccessToken(
//                 userId,
//                 empId: employee.EmployeeId,
//                 empMasterId: empMaster?.EmployeeId ?? employee.EmployeeId,
//                 userAuth?.Email ?? "",
//                 roleName,
//                 issuer,
//                 audience,
