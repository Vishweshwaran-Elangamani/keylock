...
const EmployeeSLADashboard = () => {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      case 'Open':
        return { bg: '#dbeafe', border: '#0F62FE', text: '#0F62FE', icon: Clock };
      case 'InProgress':
        return { bg: '#fef3c7', border: '#E2B93B', text: '#D4941E', icon: Zap };
      case 'Closed':
        return { bg: '#dcfce7', border: '#24A148', text: '#24A148', icon: CheckCircle };
      default:
    ...
    return (
      <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '500px' }}>
        <div className="text-center">
          ...
          <p className="text-muted mb-0" style={{ fontSize: '0.875rem' }}>Loading SLAs...</p>
        </div>
      </div>
    );
  }
    ...
    <div style={{ padding: '1.25rem 1.75rem', maxWidth: '100%', height: '100vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      {error && (
        <div className="alert alert-danger alert-dismissible fade show mb-3 d-flex align-items-start" style={{ borderRadius: cardRadius, padding: '0.75rem 1rem', border: '1px solid #fee2e2', backgroundColor: '#fef2f2' }}>
          ...
          <div className="flex-grow-1">
            ...
            <span style={{ fontSize: '0.813rem', color: '#991b1b' }}> {error}</span>
          </div>
      ...
      <div className="row g-3 mb-3">
        {[
          ...
          { label: 'In Progress', value: stats.inProgress, icon: Zap, bgColor: '#fef3c7', iconColor: '#E2B93B' },
          { label: 'Completed', value: stats.completed, icon: CheckCircle, bgColor: '#dcfce7', iconColor: '#24A148' }
        ].map((stat) => {
      ...
      {stats.overdue > 0 && (
        <div className="alert d-flex align-items-center gap-3 mb-3" style={{ backgroundColor: '#fef2f2', border: '1px solid #fecaca', borderRadius: cardRadius, padding: '0.875rem 1rem' }}>
          ...
          <div>
            <h6 className="fw-bold mb-0" style={{ color: '#991b1b', fontSize: '0.938rem' }}>
              {stats.overdue} Overdue SLA(s)
            </h6>
            <p className="mb-0" style={{ color: '#7f1d1d', fontSize: '0.813rem', marginTop: '2px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="btn-group" role="group">
          <button
            ...
            style={{ padding: '6px 12px', fontSize: '0.875rem', borderRadius: '8px 0 0 8px' }}
          >
            <Grid size={16} />
          </button>
          <button
            ...
            style={{ padding: '6px 12px', fontSize: '0.875rem', borderRadius: '0 8px 8px 0' }}
          >
            <List size={16} />
          </button>
        </div>
            ...
            <div className="table-responsive">
              <table className="table table-hover align-middle mb-0" style={{ fontSize: '0.875rem' }}>
                <thead style={{ backgroundColor: '#f8fafc' }}>
                  <tr>
                    <th style={{ padding: '1rem', fontSize: '0.813rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '2px solid #e2e8f0' }}>Type</th>
                    ...
                    <th style={{ padding: '1rem', fontSize: '0.813rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '2px solid #e2e8f0' }}>Days Left</th>
                    <th style={{ padding: '1rem', fontSize: '0.813rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '2px solid #e2e8f0' }}>Assigned To</th>
                ...
                <tbody>
                  {filteredSLAs.map((sla) => {
                    ...
                    return (
                      <tr key={sla._key} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '1rem', verticalAlign: 'middle' }}>
                          <div className="d-flex align-items-center gap-2">
                            <div style={{ width: '36px', height: '36px', backgroundColor: style.bg, borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <Icon size={18} color={style.text} />
                            </div>
                            <span className="fw-semibold" style={{ fontSize: '0.875rem', color: '#0f172a' }}>{sla.slatype || 'SLA'}</span>
                          </div>
                        </td>
                        <td style={{ padding: '1rem', verticalAlign: 'middle' }}>
                          <span className="badge" style={{
                            backgroundColor: isOverdueStatus ? '#fee2e2' : style.bg,
                        ...
                        <td style={{ padding: '1rem', fontSize: '0.875rem', verticalAlign: 'middle', color: '#475569' }}>
                          {dateHelpers.formatDeadline(sla.deadline)}
                        </td>
                        <td style={{ padding: '1rem', verticalAlign: 'middle' }}>
                          <span style={{
                            color: isOverdueStatus ? '#dc2626' : sla.daysUntilDeadline <= 3 ? '#E2B93B' : '#24A148',
                            fontWeight: 700,
                            fontSize: '0.875rem'
                          }}>
                            {isOverdueStatus ? `${Math.abs(sla.daysUntilDeadline)}d overdue` : `${sla.daysUntilDeadline}d left`}
                          </span>
                        </td>
                        <td style={{ padding: '1rem', fontSize: '0.875rem', verticalAlign: 'middle', color: '#475569' }}>
                          {sla.assignedToName || '—'}
                        ...
                        <td style={{ padding: '1rem', verticalAlign: 'middle' }}>
                          {sla.complianceStatus && (
                            <span className="badge" style={{
                              backgroundColor: sla.complianceStatus === 'OnTime' ? '#dcfce7' : sla.complianceStatus === 'Breached' ? '#fee2e2' : '#fef3c7',
                              color: sla.complianceStatus === 'OnTime' ? '#166534' : sla.complianceStatus === 'Breached' ? '#991b1b' : '#92400e',
                              fontSize: '0.75rem',
                              padding: '4px 10px',
                              fontWeight: 600,
                              borderRadius: '6px'
                        ...
                        <td style={{ padding: '1rem', textAlign: 'center', verticalAlign: 'middle' }}>
                          <button
                            ...
                            style={{ padding: '6px 12px', fontSize: '0.813rem', borderRadius: '6px', fontWeight: 600 }}
                          >
                            <Eye size={14} />
                            View
                          </button>
                  ...
                  >
                    <div className="card-body" style={{ padding: '1.125rem' }}>
                      ...
                      <div className="d-flex align-items-center gap-2 mb-3">
                        <div style={{ width: '42px', height: '42px', backgroundColor: style.bg, borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      ...
                      <div className="mb-3" style={{ fontSize: '0.813rem' }}>
                        <div className="d-flex justify-content-between mb-2 pb-2" style={{ borderBottom: '1px solid #f1f5f9' }}>
                          ...
                          <span className="fw-bold" style={{ color: '#0f172a' }}>{dateHelpers.formatDeadline(sla.deadline)}</span>
                        </div>
                        <div className="d-flex justify-content-between mb-2">
                          <span style={{ color: '#64748b', fontWeight: 600 }}>Days Remaining</span>
                          <span className="fw-bold" style={{ color: isOverdueStatus ? '#dc2626' : '#24A148' }}>
                            {isOverdueStatus ? `${Math.abs(sla.daysUntilDeadline)}d overdue` : `${sla.daysUntilDeadline}d left`}
                          </span>
                        </div>
                        {sla.assignedToName && (
                          <div className="d-flex justify-content-between mb-2">
                            ...
                            <span className="fw-semibold text-truncate" style={{ color: '#0f172a', maxWidth: '55%' }}>{sla.assignedToName}</span>
                          </div>
                        )}
                        {sla.complianceStatus && (
                          <div className="d-flex justify-content-between">
                            <span style={{ color: '#64748b', fontWeight: 600 }}>Compliance</span>
                            <span className="badge" style={{
                              backgroundColor: sla.complianceStatus === 'OnTime' ? '#dcfce7' : sla.complianceStatus === 'Breached' ? '#fee2e2' : '#fef3c7',
                              color: sla.complianceStatus === 'OnTime' ? '#166534' : sla.complianceStatus === 'Breached' ? '#991b1b' : '#92400e',
                              fontSize: '0.75rem',
                      ...
                      <button
                        ...
                        onClick={() => handleViewDetails(sla.slaid)}
                        style={{ padding: '8px', fontSize: '0.875rem', fontWeight: 600, borderRadius: '6px' }}
                      >
                        <Eye size={16} />
                        View Details
                      </button>
