...
import {
  CheckCircle,Send,AlertTriangle,Target,Users, Loader,Star,Home,
} from "lucide-react";
import { useNavigate, Link } from "react-router-dom";
import {
  orgGoalFeedbackApi,peerQueueApi,employeeApi,goalsApi,
...
const CustomSelect = ({
  ...
  return (
    <div className="scf-dropdown-wrapper">
      <button type="button" id={id} onClick={() => !disabled && setIsOpen((o) => !o)}
        className={`scf-dropdown-select ${  isOpen ? "scf-dropdown-select--open" : "" }`} disabled={disabled} >    
      ...
      {isOpen && (
        <ul className="scf-dropdown-list">
          {options.map((opt) => {
            ...
            return (
              <li key={opt.value} className={`scf-dropdown-option ${ isSelected ? "scf-dropdown-option--selected" : ""}`}
...
const { baseDashboard, feedbackDashboard, baseLabel } = useMemo(
  ...
  const [goalForm, setGoalForm] = useState({
    organizationObjectiveId: "", objectiveTitle: "", rating: 4,feedbackComments: "",isAnonymous: false,
  });
  const [contextForm, setContextForm] = useState({
    recipientEmployeeId: "",  recipientName: "", projectContext: "",feedbackContent: "",isAnonymous: false,});
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          const employeesList = Array.isArray(empResponse.data) ? empResponse.data : empResponse.data?.data || empResponse.data?.$values || [];
  ...
  const handleObjectiveChange = (selectedIdStr) => {
    ...
    setGoalForm((prev) => ({
      ...prev,organizationObjectiveId: selectedId,objectiveTitle:selectedObjective?.goalName ||selectedObjective?.title ||
        selectedObjective?.name || selectedObjective?.goalTitle ||"", }));
  };
  const submitGoal = async (e) => {
    ...
    try {
      const payload = {
        goalId: Number(goalForm.organizationObjectiveId), submittedByEmployeeId: Number(user?.empId),
        recipientEmployeeId: Number(user?.empId),rating: Number(goalForm.rating || 0),
        feedbackComments: goalForm.feedbackComments,isAnonymous: !!goalForm.isAnonymous,
      };
...
const handleEmployeeChange = (selectedIdStr) => {
    ...
    setContextForm((prev) => ({
      ...prev, recipientEmployeeId: selectedId, recipientName: selectedEmployee
  ...
  const submitContext = async (e) => {
    ...
    try {
      const contextPrefix = contextForm.projectContext
        ...
        : "";
      const peerPayload = {
        submittedByEmployeeId: Number(user?.empId),recipientEmployeeId: Number(contextForm.recipientEmployeeId),
        feedbackContent: contextPrefix + contextForm.feedbackContent,isAnonymous: !!contextForm.isAnonymous, };
      ...
      if (createResponse?.data?.success || createResponse?.success) {
        ...
        setContextForm({
          recipientEmployeeId: "", recipientName: "",projectContext: "",
          feedbackContent: "",isAnonymous: false,
      });
      setTimeout(() => setSuccessMsg(""), 5000);
      } else {
        setError(
          createResponse?.data?.message || createResponse?.message || "Failed to submit context feedback"
        );
      }
    } catch (err) {
      console.error("Context feedback submission error:", err);
      setError(
        err?.response?.data?.message || err.message || "Failed to submit context feedback."
      );
    } finally {
      setLoading(false);
    }
  ...
  const renderStars = (rating) => {
    return [...Array(5)].map((_, index) => {
      ...
      return (
        <Star key={index} size={20}  fill={starValue <= rating ? "#ffc107" : "none"}
          stroke={starValue <= rating ? "#ffc107" : "#cbd5e1"}
          strokeWidth={2}  className="scf-star" onClick={() => setGoalForm({ ...goalForm, rating: starValue })}/>
      );
    });
  };
  const getGoalDisplayName = (goal) => {
    return (
      goal.goalName ||goal.title ||goal.name ||goal.goalTitle ||goal.objectiveName ||`Untitled Goal`
    );
  };
      ...
      <div className="scf-content">
        <nav className="scf-breadcrumb" aria-label="breadcrumb">
          <ol className="scf-breadcrumb-list">
            <li className="scf-breadcrumb-item">
              <Link to={baseDashboard} className="scf-breadcrumb-link" aria-label="Dashboard" title="Dashboard" >
                <Home size={18} className="scf-breadcrumb-icon" />
              </Link>
            </li>
            <li className="scf-breadcrumb-separator">/</li>
            <li className="scf-breadcrumb-item">
              <Link to={feedbackDashboard} className="scf-breadcrumb-link"> Feedback Management </Link>
            </li>
            <li className="scf-breadcrumb-separator">/</li>
        ...
        {error && (
          <div className="scf-alert scf-alert-error">
            ...
            <button type="button" className="scf-alert-close" onClick={() => setError("")}>
              ×
            </button>
          </div>
        )}
        {successMsg && (
          <div className="scf-alert scf-alert-success">
            ...
            <button type="button"className="scf-alert-close"onClick={() => setSuccessMsg("")} >
              ×
            </button>
          </div>
        )}
        <div className="scf-tabs">
        <button type="button" onClick={() => setActiveTab("goal")} className={`scf-tab-button ${  activeTab === "goal" ? "scf-tab-button--active" : "" }`} >
        <Target size={16} />Goal Feedback  </button>
          <button type="button" onClick={() => setActiveTab("context")} className={`scf-tab-button ${
              activeTab === "context" ? "scf-tab-button--active" : ""}`}>
            <Users size={16} /> Context Feedback
          </button>
        </div>
        {loadingData && (
            ...
            <div className="scf-card-body">
              <form onSubmit={submitGoal}>
                <div className="scf-form-group">
                  ...
                  <CustomSelect id="goalSelect"  value={goalForm.organizationObjectiveId}  onChange={handleObjectiveChange}
                    disabled={false}  placeholder={ objectives.length === 0
                        ? "No organization goals available" : "Choose a goal..."}
                    options={objectives.map((obj) => {
                      const objId = obj.goalId || obj.objectiveId || obj.id;
                      return {
                        value: objId, label: getGoalDisplayName(obj),
                      };
                    })}
                  />
                </div>
                {selectedObjective && (
                  <div className="scf-info-box">
                    ...
                    <p className="scf-info-text">
                      {selectedObjective.description || selectedObjective.goalDescription || selectedObjective.objectiveDescription || "No description available"}
                    </p>
                  </div>
                )}
                <div className="scf-form-group">
                ...
                <div className="scf-form-group">
                  ...
                  <textarea className="scf-textarea"
                    rows={5} value={goalForm.feedbackComments} onChange={(e) => setGoalForm((prev) => ({ ...prev,feedbackComments: e.target.value,
                  }))}
                    placeholder="Provide your detailed feedback on this goal..." required maxLength={1000}/>
                  <div className="scf-textarea-footer">
                    <span className="scf-textarea-hint"> Be specific and constructive</span>
                    <span className={`scf-textarea-count ${goalForm.feedbackComments.length > 900? "scf-textarea-count--warning": ""}`}>
                ...
                <div className="scf-form-group">
                  <div className="scf-checkbox">
                    <input className="scf-checkbox-input" type="checkbox"id="goalAnon"
                      checked={goalForm.isAnonymous} onChange={(e) =>setGoalForm((prev) => ({  ...prev, isAnonymous: e.target.checked, }))}/>
                    <label className="scf-checkbox-label" htmlFor="goalAnon"> Submit anonymously</label>
                  </div>
                </div>
                <button type="submit" className="scf-submit-button" disabled={loading || !goalForm.organizationObjectiveId} >
                  {loading ? (
                    <>
                      <Loader size={16} className="scf-loader" /> Submitting...
                    </>
                  ) : (
                    <>
          ...
          <div className="scf-card">
            <div className="scf-card-body">
              <form onSubmit={submitContext}>
                <div className="scf-form-group">
                  <label className="scf-label">Select Recipient <span className="scf-required">*</span> </label>
                  <CustomSelect id="recipientSelect" value={contextForm.recipientEmployeeId}
                    onChange={handleEmployeeChange} disabled={false} placeholder={
                      employees.length === 0 ? "No employees available" : "Choose a team member..." }
                    options={employees.map((emp) => ({ value: emp.employeeId,label: `${emp.firstName} ${emp.lastName} (${emp.email})`,
                    }))}
                  />
                </div>
                {selectedEmployee && (
                  <div className="scf-info-box">
                    ...
                    <div className="scf-employee-name">{selectedEmployee.firstName} {selectedEmployee.lastName}</div>
                    <small className="scf-employee-email">{selectedEmployee.email}</small>
                  </div>
                )}
                <div className="scf-form-group">
                  <label className="scf-label">Project Context <span className="scf-required">*</span> </label>
                  <input type="text" className="scf-input" value={contextForm.projectContext}
                    onChange={(e) => setContextForm((prev) => ({ ...prev, projectContext: e.target.value,
                }))}  placeholder="e.g., AI Platform Project, Q4 Sprint" required />
            </div>
                <div className="scf-form-group">
                  <label className="scf-label"> Feedback Content <span className="scf-required">*</span></label>
                  <textarea className="scf-textarea" rows={5} value={contextForm.feedbackContent}
                    onChange={(e) =>setContextForm((prev) => ({  ...prev,  feedbackContent: e.target.value,}))}
                    placeholder="Provide constructive feedback on their work, collaboration, or skills..."
                    required maxLength={1000}/>
                  <div className="scf-textarea-footer">
                    <span className="scf-textarea-hint"> Be specific and constructive</span>
                    <span className={`scf-textarea-count ${contextForm.feedbackContent.length > 900
                ...
                <div className="scf-form-group">
                  <div className="scf-checkbox">
                    <input  className="scf-checkbox-input"  type="checkbox"  id="contextAnon"
                    checked={contextForm.isAnonymous} onChange={(e) => setContextForm((prev) => ({
                    ...prev,isAnonymous: e.target.checked,
                        }))
                      } />
                    <label className="scf-checkbox-label" htmlFor="contextAnon">Submit anonymousl</label>
                  </div>
                </div>
                <button type="submit" className="scf-submit-button"
