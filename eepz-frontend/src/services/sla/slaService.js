...
export const escalationHelpers = {
  ...
  getEscalationBlockReason: (sla, escalations = [], level = 'L2') => {
    ...
    return hasEscalation ? `⚠️ Already escalated to ${level}. Wait for resolution.` : null;
  },
...
export const dateHelpers = {
  ...
  getUrgencyStatus: (deadline) => {
    const days = dateHelpers.daysRemaining(deadline);
    if (days < 0) return { status: 'Overdue', color: '#E01950', icon: '🔴' };
    if (days === 0) return { status: 'Today', color: '#E2B93B', icon: '🟠' };
    if (days <= 3) return { status: 'Urgent', color: '#E2B93B', icon: '⚠️' };
    if (days <= 7) return { status: 'Soon', color: '#0F62FE', icon: '🟡' };
...
const slaService = {
  ...
  createBulkSLA: async (data) => {
    try {
      ...
      return await slaApi.post('/create-bulk', payload);
    } catch (error) {
  ...
  submitEscalation: async (data) => {
    try {
      console.log('⚠️ L1 Escalation (Employee → Manager)');
      const payload = {
        slaid: data.slaid,
        reason: data.reason,
  ...
  escalateToDeptHead: async (data) => {
    try {
      console.log('⚠️⚠️ L2 Escalation (Manager → Dept Head)');
      const payload = {
        slaid: data.slaid,
        reason: data.reason,
        description: data.description || '',
  ...
  getDepartmentHeadForEscalation: async (managerId) => {
    try {
      ...
      if (validDeptHeads.length === 0) {
        console.warn(`⚠️ Manager's reporting manager (${managerReportsTo}) is NOT in dept heads`);
