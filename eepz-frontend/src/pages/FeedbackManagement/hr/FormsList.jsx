...
import { Eye, Trash2, Plus, RefreshCw, AlertTriangle, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="container-fluid py-4" style={{ maxWidth: '1400px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h3 className="fw-bold mb-1" style={{ color: 'var(--color-primary-1)' }}>📋 Forms Management</h3>
      ...
      {/* FORMS TABLE */}
      {forms.length === 0 ? (
        <div className="card border-0 text-center py-5" style={{ border: '1px solid var(--border)' }}>
        ...
        <div className="card border-0" style={{ border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
          <div className="table-responsive">
            <table className="table table-hover mb-0">
              <thead style={{ background: '#f9f9fa', borderBottom: '2px solid var(--border)' }}>
                <tr>
                  <th style={{ color: 'var(--color-primary-1)', fontWeight: '600' }}>Form Name</th>
                  <th style={{ color: 'var(--color-primary-1)', fontWeight: '600' }}>Type</th>
                  <th style={{ color: 'var(--color-primary-1)', fontWeight: '600' }} className="text-center">Deadline</th>
                  <th style={{ color: 'var(--color-primary-1)', fontWeight: '600' }} className="text-center">Actions</th>
                </tr>
              </thead>
              <tbody>
                {forms.map(form => {
                  const totalDist = form.distributedToEmployeeIds?.length || form.totalDistributed || 0;
                  const totalResp = form.totalResponses || responses[form.formId]?.length || 0;
                  const rate = totalDist > 0 ? Math.round((totalResp / totalDist) * 100) : 0;
                        ...
                        <tr style={{ background: '#f0f4ff' }}>
                          <td colSpan="7" className="py-3">
                            <h6 className="fw-bold mb-3">
                              <CheckCircle size={16} className="me-2" style={{ display: 'inline', color: '#24A148' }} />
                              Responses ({totalResp})
                            </h6>
                                  ...
                                  <tbody>
                                    {responses[form.formId]?.map(resp => (
                                      <tr key={resp.responseId}>
                                        <td className="fw-600">{resp.employeeName || 'Employee'}</td>
