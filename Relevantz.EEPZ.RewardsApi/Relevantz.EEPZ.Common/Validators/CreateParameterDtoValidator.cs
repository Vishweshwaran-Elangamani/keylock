...
{
    public class CreateParameterDtoValidator : AbstractValidator<CreateParameterDto>
    {
        public CreateParameterDtoValidator()
        {
            RuleLevelCascadeMode = CascadeMode.Stop;

            RuleFor(x => x.RewardTypeId)
                .GreaterThan(0)
                .WithMessage("RewardTypeId must be greater than 0.");
