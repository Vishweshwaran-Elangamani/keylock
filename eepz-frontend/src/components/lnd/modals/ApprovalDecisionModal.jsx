...
const ApprovalDecisionModal = ({ approval, onClose, onSuccess }) => {
  ...
  const handleDownload = async () => {
    try {
      const response = await lndService.downloadApprovalAttachment(approval.approvalId);
  ...
  const handleSubmit = async (e) => {
    ...
    if (approval.approvalType === APPROVAL_TYPE.ASSIGNMENT_COMPLETION && decision === 'approve') {
      if (!newRating || isNaN(newRating) || Number(newRating) < 1 || Number(newRating) > 10) {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(`Approval ${decision === 'approve' ? 'approved' : 'rejected'} successfully!`);
        onSuccess();
      } else {
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ padding: '1.5rem' }}>
              ...
              {approval.approvalType === APPROVAL_TYPE.ASSIGNMENT_COMPLETION && decision === 'approve' && (
              ...
              {approval.approvalType !== APPROVAL_TYPE.SME_REQUEST && (
                <div style={{ marginBottom: '1.5rem' }}>
                  ...
                  <textarea
                    ...
                    rows={4}
                    required={approval.approvalType !== APPROVAL_TYPE.SME_REQUEST}
                    style={{
                      width: '100%',
                      padding: '0.75rem',
                      border: '1px solid #e5e7eb',
