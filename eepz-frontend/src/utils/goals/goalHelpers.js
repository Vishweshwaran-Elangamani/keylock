...
export const getTaskStatus = (progress, hasPendingApproval, isAcknowledged) => {
  ...
  if (progress === 100 && hasPendingApproval)
    return TASK_STATUS.PENDING_ACKNOWLEDGMENT;
...
export const canUserComment = (goal, currentUser) => {
  ...
  if (
    goal.assignees &&
    goal.assignees.some((a) => a.employeeMasterId === currentUser.id)
  ) {
    return true;
  }
...
export const filterGoals = (goals, filters) => {
  ...
  return goals.filter((goal) => {
    ...
    if (
      filters.dateFrom &&
      isBefore(parseISO(goal.createdOn), parseISO(filters.dateFrom))
    ) {
      return false;
    }
    if (
      filters.dateTo &&
      isAfter(parseISO(goal.createdOn), parseISO(filters.dateTo))
    ) {
      return false;
    }
