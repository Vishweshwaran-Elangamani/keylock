...
const RSVPSummary = () => {
  ...
  const responsePercentage = totalInvitations > 0 
            ...
            <div className="col-xl-2 col-lg-4 col-md-4 col-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-body text-center p-3">
                  ...
                  <div className="fw-bold fs-4 mb-1" style={{ color: '#1e293b' }}>
                    {totalInvitations}
                  </div>
                ...
                <div className="progress mb-3" style={{ height: '30px' }}>
                  {acceptedCount > 0 && (
                    <div 
                      ...
                      style={{ width: `${(acceptedCount / totalInvitations) * 100}%` }}
                    >
                      {acceptedCount > 0 && <small className="fw-semibold">{acceptedCount}</small>}
                    </div>
                  )}
                  {tentativeCount > 0 && (
                    <div 
                      className="progress-bar bg-info d-flex align-items-center justify-content-center" 
                      style={{ width: `${(tentativeCount / totalInvitations) * 100}%` }}
                    >
                      {tentativeCount > 0 && <small className="fw-semibold text-dark">{tentativeCount}</small>}
                    </div>
                  )}
                  {declinedCount > 0 && (
                    <div 
                      className="progress-bar bg-danger d-flex align-items-center justify-content-center" 
                      style={{ width: `${(declinedCount / totalInvitations) * 100}%` }}
                    >
                      {declinedCount > 0 && <small className="fw-semibold">{declinedCount}</small>}
                    </div>
                  )}
                  {pendingCount > 0 && (
                    <div 
                      className="progress-bar bg-secondary d-flex align-items-center justify-content-center" 
                      style={{ width: `${(pendingCount / totalInvitations) * 100}%` }}
                    >
                      {pendingCount > 0 && <small className="fw-semibold">{pendingCount}</small>}
                    </div>
                  )}
                </div>
                ...
                <div className="table-responsive">
                  <table className="table table-hover align-middle mb-0">
                    <thead className="table-light">
                      <tr>
                        <th className="fw-semibold" style={{ color: '#64748b' }}>Employee Name</th>
                        <th className="fw-semibold" style={{ color: '#64748b' }}>Status</th>
                        <th className="fw-semibold" style={{ color: '#64748b' }}>Response Date</th>
                        <th className="fw-semibold" style={{ color: '#64748b' }}>Comments</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={participant.participantId || index}>
                          <td>
                            <div className="d-flex align-items-center gap-2">
                              ...
                              <span className="fw-semibold">{participant.employeeName || 'Unknown'}</span>
                            </div>
                          </td>
                          <td>
                            {getStatusBadge(participant.rsvpStatus)}
                          </td>
                          <td>
                          ...
                          <td>
                            {participant.rsvpComments ? (
                              <div className="d-flex align-items-start gap-2">
                                <MessageSquare size={16} className="text-muted mt-1" style={{ flexShrink: 0 }} />
                                <small className="text-muted">{participant.rsvpComments}</small>
                              </div>
                            ) : (
