...
export default function SelfAssessmentForm() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/AppraisalProcess/employee/${id}`)
      .then(({ data }) => {
        if (!mounted) return;
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "16px", fontSize: "14px" }}>
        Form: <b>{model.form?.name || model.formId}</b> • User:{" "}
        <b>{model.userId}</b> • Submitted:{" "}
        <b>
          {model.submittedAt
            ? new Date(model.submittedAt).toLocaleString()
            : "-"}
        </b>
      </div>
      ...
      >
        ...
        <tbody>
          {model?.map((d) => (
            <tr key={d.detailId}>
              <td style={tdStyle}>
                {d.competency?.name ?? `Competency #${d.competencyId}`}
              </td>
              ...
              <td style={tdStyle}>
                <select
                  ...
                  onChange={(e) =>
                    setAnswer(d.detailId, "employeeRating", e.target.value)
                  }
                  required
                  style={{
                    width: "140px",
              ...
              <td style={tdStyle}>
                <textarea
                  ...
                  onChange={(e) =>
                    setAnswer(d.detailId, "employeeComments", e.target.value)
                  }
