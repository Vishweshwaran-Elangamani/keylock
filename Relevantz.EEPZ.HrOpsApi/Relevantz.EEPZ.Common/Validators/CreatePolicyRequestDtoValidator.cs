...
{
    public class CreatePolicyRequestDtoValidator : AbstractValidator<CreatePolicyRequestDto>
    {
        public CreatePolicyRequestDtoValidator()
        {
            ...
            RuleFor(x => x.Status)
                .Must(status => string.IsNullOrEmpty(status) || new[] { "Draft", "Published", "Archived" }.Contains(status))
