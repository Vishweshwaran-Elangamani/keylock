...
const careerProgressionService = {
  //  GET: All promotions - SIMPLE & WORKING
  ...
  //  NEW: GET promotions submitted to leadership (Leadership only - VIEW ONLY)
  getSubmittedToLeadership: async () => {
    try {
      console.log(" Fetching promotions submitted to leadership");
      const response = await hrApi.get(
        `/CareerProgression/submitted-to-leadership`
      );
      console.log(" Response:", response.data);
      ...
      console.error(" Error fetching submitted promotions:", error);
      return [];
    }
  },
};
