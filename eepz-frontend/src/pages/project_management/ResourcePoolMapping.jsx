// src/pages/ProjectManagement/ResourcePoolMapping.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import {
  Users, ArrowLeft, CheckCircle, AlertCircle, Search, Home, UserCheck, Plus, X, Info,
  RefreshCw, Database, UserX, Check, Download
} from 'lucide-react';
...
const ResourcePoolMapping = () => {
  ...
  // Status helpers
  const isEmployeeSelected = empId => selectedEmployees.some(emp => emp.employeeMasterId === empId);
  const isEmployeeInResourcePool = empId => resourcePoolMappedEmployees.some(emp => emp.employeeMasterId === empId);
  const isEmployeeMappedToAny = empId => mappedEmployees.some(emp => emp.employeeMasterId === empId);
  ...
  // Counts
  const availableCount = filteredEmployees.filter(emp => getEmployeeStatus(emp.employeeMasterId).text === 'Available').length;
  ...
  const handleAddEmployee = employee => {
    ...
    if (status.text === 'Available' && !isEmployeeSelected(employee.employeeMasterId)) {
      ...
      setMessage({ type: 'info', text: `${employee.firstName} ${employee.lastName} added to selection` });
      setTimeout(() => setMessage(null), 1800);
    } else {
      setMessage({ type: 'warning', text: `${employee.firstName} ${employee.lastName} is unavailable for mapping` });
  ...
  const handleSelectAll = () => {
    const canAdd = filteredEmployees.filter(emp => getEmployeeStatus(emp.employeeMasterId).text === 'Available');
    const addable = canAdd.filter(emp => !isEmployeeSelected(emp.employeeMasterId));
    if (addable.length > 0) {
      setSelectedEmployees(prev => [...prev, ...addable]);
      setMessage({ type: 'success', text: `${addable.length} employees added to selection` });
      setTimeout(() => setMessage(null), 1800);
    }
  };

  const handleClearAll = () => {
    if (selectedEmployees.length > 0 && window.confirm('Clear all selections?')) {
  ...
  const handleMapToResourcePool = async () => {
    if (selectedEmployees.length === 0) {
      setMessage({ type: 'error', text: 'Please select at least one employee to map' });
      setTimeout(() => setMessage(null), 2200);
      return;
    }
    ...
    if (
      !window.confirm(
        `Confirm mapping ${selectedEmployees.length} employee(s) to Resource Pool?\n\n` +
          ...
          `Selected: ${selectedEmployees.map(e => `${e.firstName} ${e.lastName}`).join(', ')}`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setResourcePoolMappedEmployees(prev =>
        [...prev, ...newlyMappedEmployees.filter(ne => !prev.some(p => p.employeeMasterId === ne.employeeMasterId))]
      );
      ...
      setMessage({
        type: 'success',
        text: `Successfully mapped ${response.data?.mappedCount || employeeIds.length} employee(s) to Resource Pool. They have been removed from the list.`,
        isLong: true
      });
      ...
      setResourcePoolMappedEmployees(prev => prev.filter(emp => !employeeIds.includes(emp.employeeMasterId)));
      setMappedEmployees(prev => prev.filter(emp => !employeeIds.includes(emp.employeeMasterId)));
  ...
  return (
    <div className="h-100 d-flex flex-column">
      ...
      <nav aria-label="breadcrumb" className="mb-3">
        <ol className="breadcrumb mb-0 p-3 rounded" style={{ backgroundColor: 'rgba(151, 36, 126, 0.05)', fontSize: '0.875rem' }}>
          <li className="breadcrumb-item">
            <a
              href="#"
              onClick={(e) => { e.preventDefault(); navigate('/hr/dashboard/projectmgmt'); }}
              style={{ color: 'var(--color-primary-3)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.25rem' }}
            >
      ...
      <div className="d-flex align-items-center justify-content-between mb-4">
        <div className="d-flex align-items-center gap-3">
          <button className="btn btn-link text-decoration-none p-0" onClick={() => navigate('/hr/dashboard/projectmgmt/list')}>
            <ArrowLeft size={24} />
          </button>
          ...
          <div>
            ...
            <p className="text-muted mb-0 small">
              Manage employee mappings to {RESOURCE_POOL_PROJECT_NAME} (employees mapped now will disappear from the list)
            </p>
          </div>
        </div>
        <div className="d-flex gap-2">
          <button className="btn btn-outline-secondary d-flex align-items-center gap-2" onClick={handleRefresh} disabled={isRefreshing}>
            {isRefreshing ? <RefreshCw size={16} className="animate-spin" /> : <RefreshCw size={16} />} Refresh
          </button>
          <button
            className="btn btn-primary d-flex align-items-center gap-2"
            onClick={() => navigate(`/hr/dashboard/projectmgmt/details/${RESOURCE_POOL_PROJECT_NAME.toLowerCase().replace(/\./g, '-')}`)}
          >
      ...
      {message && (
        <div
          className={`alert alert-${message.type === 'success' ? 'success' : message.type === 'info' ? 'info' : message.type === 'warning' ? 'warning' : 'danger'} d-flex align-items-center gap-2 mb-4 ${message.isLong ? 'alert-dismissible fade show' : ''}`}
          role="alert"
        >
          {message.type === 'success' && <CheckCircle size={20} />}
          {message.type === 'info' && <Info size={20} />}
          {message.type === 'warning' && <AlertCircle size={20} className="text-warning" />}
      ...
      {error && (
        <div className="alert alert-danger d-flex align-items-center gap-2 mb-4" role="alert">
          ...
          <div>
            ...
            <button className="btn btn-sm btn-outline-danger ms-3" onClick={fetchInitialData}>Retry</button>
          </div>
        </div>
      )}
      ...
      <div className="row g-4 h-100">
                ...
                <table className="table table-hover mb-0">
                  <thead className="table-light sticky-top">
                    <tr>
                      ...
                      <th className="py-3 text-center" style={{ width: '90px' }}>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredEmployees.length === 0 ? (
                      <tr>
                        <td colSpan="5" className="text-center py-5 text-muted">
                          ...
                          <p className="small mb-0">{searchTerm ? 'No matches for your search.' : 'All eligible employees are mapped.'}</p>
                        </td>
                      </tr>
                    ) : (
                        ...
                        return (
                          <tr key={emp.employeeMasterId}>
                            <td className="py-2 px-3 align-middle">
                              ...
                              <small className="text-muted d-block">{emp.employeeCompanyId}</small>
                              <small className="text-muted">{emp.email}</small>
                            </td>
                            <td className="py-2 align-middle">
                              <span className="badge bg-light text-dark fs-6">{emp.roleName}</span>
                            </td>
                            <td className="py-2 align-middle">
                              <small className="text-muted">{emp.departmentName}</small>
                            </td>
                            <td className="py-2 text-center align-middle">
                              <span className={`badge ${status.badge}`}>
                                {React.createElement(status.icon, { size: 12, className: 'me-1' })} {status.text}
                              </span>
                            </td>
                            <td className="py-2 text-center align-middle">
                              <button
                                className={`btn btn-sm ${isEmployeeSelected(emp.employeeMasterId) ? 'btn-outline-success' : isUnavailable ? 'btn-outline-secondary' : 'btn-success'}`}
                              ...
                              >
                                {isEmployeeSelected(emp.employeeMasterId) ? <Check size={16} /> : isUnavailable ? <X size={16} /> : <Plus size={16} />}
                              </button>
                            </td>
                          </tr>
                        );
        ...
        <div className="col-lg-4">
          <div className="card border-0 shadow-sm h-100 d-flex flex-column">
            <div className="card-header bg-light d-flex align-items-center justify-content-between">
              <h5 className="mb-0 d-flex align-items-center gap-2">
                <UserCheck size={20} className="text-success" /> Selected Employees ({selectedEmployees.length})
              </h5>
              {selectedEmployees.length > 0 && (
                <div className="btn-group btn-group-sm" role="group">
                  <button className="btn btn-outline-secondary" onClick={handleClearAll} title="Clear all">
                    <X size={14} />
                  </button>
                </div>
              )}
            ...
            <div className="card-body d-flex flex-column flex-grow-1 p-0">
              <div
                ...
                style={{ minHeight: '300px', maxHeight: '400px', overflowY: 'auto', backgroundColor: '#f8f9fa' }}
              >
                {selectedEmployees.length === 0 ? (
                  <div className="text-center text-muted py-5">
                    <Users size={48} className="mb-3 opacity-25" />
                    <p className="mb-2">No employees selected</p>
                    <small className="text-muted">Select available employees from the left to add here</small>
                  </div>
                ) : (
                    ...
                    {selectedEmployees.map((emp, index) => (
                      ...
                      >
                        <div className="flex-grow-1">
                          <div className="fw-semibold small">{index + 1}. {emp.firstName} {emp.lastName}</div>
              ...
              <div className="p-3 border-top bg-light">
                <button
                  ...
                  title={selectedEmployees.length === 0 ? 'Select employees first' : isSubmitting ? 'Mapping...' : 'Map to Resource Pool'}
                >
                  {isSubmitting ? (
                    <>
                ...
                {selectedEmployees.length > 0 && (
                  <small className="text-muted d-block text-center mt-2">
                    Backend enforces: single-project rule, L2 manager assignment, primary status
                  </small>
                )}
              </div>
            </div>
