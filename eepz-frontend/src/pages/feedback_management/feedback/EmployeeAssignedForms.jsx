...
import { RefreshCw, AlertTriangle, Eye, FileText, Calendar, Loader, Lock } from 'lucide-react';
...
export default function EmployeeAssignedForms() {
  ...
  const fetchForms = useCallback(async (retryCount = 0) => {
    if (!user?.empId) {
    ...
    try {
      ...
      if (err.response?.status === 404) msg = 'Endpoint not found: /HrFeedbackForm/forms';
  ...
  const fetchSubmittedForms = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/HrFeedbackForm/responses/by-employee/${user.empId}`, {
        timeout: 8000
      });
      if (res.status === 200 && Array.isArray(res.data?.data)) {
  ...
  const stats = useMemo(() => {
    ...
    const remaining = allForms.filter(f => !submittedFormIds.has(f.formId)).length;
  ...
  return (
    <div className="container-fluid py-4" style={{ maxWidth: '1200px' }}>
      ...
      <div className="d-flex justify-content-between align-items-start mb-4">
        <div>
          <h3 className="fw-bold mb-1" style={{ color: 'var(--color-primary-1)' }}>
            All Forms
          </h3>
          <p className="mb-0 small text-muted">
            Every form is listed here regardless of its status
            {lastFetchTime && <span className="ms-2">(Last updated: {lastFetchTime})</span>}
          </p>
        </div>
        <button
      ...
      <div className="row g-3 mb-4">
        ...
        <div className="col-6 col-md-4">
          <div className="card border-0 text-center" style={{ border: '1px solid var(--border)' }}>
            <div className="card-body">
              <h5 className="fw-bold" style={{ color: '#24A148' }}>{stats.submitted}</h5>
        ...
        <div className="col-6 col-md-4">
          <div className="card border-0 text-center" style={{ border: '1px solid var(--border)' }}>
            <div className="card-body">
              <h5 className="fw-bold" style={{ color: '#0F62FE' }}>{stats.remaining}</h5>
      ...
      {loading ? (
        <div className="text-center py-5">
          <Loader size={32} className="mb-3 text-primary" style={{ animation: 'spin 1s linear infinite', display: 'block' }} />
          <p className="text-muted">Loading forms...</p>
        </div>
      ) : allForms.length === 0 ? (
        <div className="card border-0" style={{ border: '1px solid var(--border)' }}>
          <div className="card-body text-center py-5">
            <FileText size={48} className="text-muted mb-3" style={{ opacity: 0.3 }} />
            <h5 className="text-muted">No Forms Available</h5>
            <p className="text-muted small">There are currently no forms to display.</p>
            <button className="btn btn-outline-primary btn-sm mt-2" onClick={handleRefresh}>Refresh</button>
          </div>
        </div>
      ) : (
            ...
            return (
              <div key={form.formId} className="col-md-6 col-lg-4">
                <div 
                  ...
                  onMouseEnter={(e) => !isSubmitted && (e.currentTarget.style.transform = 'translateY(-4px)')}
                  onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}
                >
                  <div className="card-body d-flex flex-column">
                    {/* Title */}
                    <div className="d-flex justify-content-between align-items-start mb-3">
                      <h6 className="fw-bold mb-0" style={{ flex: 1, marginRight: 8 }}>
                        {form.formName}
                      </h6>
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="fw-600 small">{getFormTypeLabel(form.formType)}</div>
                    </div>

                    {/* Deadline */}
                    <div className="mb-3 p-2 rounded" style={{ backgroundColor: '#f9f9fa' }}>
                    ...
                    <div className="d-grid">
                      {isSubmitted ? (
                        <button
                          ...
                          style={{ borderRadius: 'var(--radius-md)', cursor: 'not-allowed' }}
                          disabled
                        >
