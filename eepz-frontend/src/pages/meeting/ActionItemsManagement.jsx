...
const ActionItemsManagement = () => {
  ...
  const markAsCompleted = async () => {
    ...
    try {
      await momService.updateActionItemStatus(selectedItem.actionItemId, 'Completed');
  ...
  const filteredItems = actionItems.filter(item => {
    const matchesSearch = item.taskDescription?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const stats = {
    ...
    overdue: actionItems.filter(i => i.status === 'Pending' && new Date(i.dueDate) < new Date()).length
  };
    ...
    <div className="container-fluid px-4 py-4" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div className="row g-3 mb-4">
        <div className="col-lg-3 col-md-6">
          <div
            className={`card border-0 shadow-sm h-100 ${filter === 'all' ? 'border-primary' : ''}`}
            onClick={() => setFilter('all')}
            style={{ cursor: 'pointer', transition: 'all 0.2s', borderWidth: filter === 'all' ? '2px' : '0' }}
          >
            <div className="card-body d-flex align-items-center p-3">
              ...
              >
                <i className="bi bi-card-checklist fs-4" style={{ color: '#1976d2' }}></i>
              </div>
              <div>
                <h4 className="fw-bold mb-0" style={{ fontSize: '1.5rem', color: '#1e293b' }}>
                  {stats.total}
                </h4>
        ...
        <div className="col-lg-3 col-md-6">
          <div
            className={`card border-0 shadow-sm h-100 ${filter === 'pending' ? 'border-warning' : ''}`}
            onClick={() => setFilter('pending')}
            style={{ cursor: 'pointer', transition: 'all 0.2s', borderWidth: filter === 'pending' ? '2px' : '0' }}
          >
            <div className="card-body d-flex align-items-center p-3">
              <div
                className="rounded-circle d-flex align-items-center justify-content-center me-3"
              ...
              >
                <i className="bi bi-clock-history fs-4" style={{ color: '#f57c00' }}></i>
              </div>
              <div>
                <h4 className="fw-bold mb-0" style={{ fontSize: '1.5rem', color: '#1e293b' }}>
                  {stats.pending}
        ...
        <div className="col-lg-3 col-md-6">
          <div
            className={`card border-0 shadow-sm h-100 ${filter === 'completed' ? 'border-success' : ''}`}
            onClick={() => setFilter('completed')}
            style={{ cursor: 'pointer', transition: 'all 0.2s', borderWidth: filter === 'completed' ? '2px' : '0' }}
          >
            <div className="card-body d-flex align-items-center p-3">
              <div
                className="rounded-circle d-flex align-items-center justify-content-center me-3"
              ...
              >
                <i className="bi bi-check-circle fs-4" style={{ color: '#388e3c' }}></i>
              </div>
              <div>
                <h4 className="fw-bold mb-0" style={{ fontSize: '1.5rem', color: '#1e293b' }}>
                  {stats.completed}
        ...
        <div className="col-lg-3 col-md-6">
          <div
            className={`card border-0 shadow-sm h-100 ${filter === 'overdue' ? 'border-danger' : ''}`}
            onClick={() => setFilter('overdue')}
            style={{ cursor: 'pointer', transition: 'all 0.2s', borderWidth: filter === 'overdue' ? '2px' : '0' }}
          >
            <div className="card-body d-flex align-items-center p-3">
              <div
                className="rounded-circle d-flex align-items-center justify-content-center me-3"
              ...
              >
                <i className="bi bi-exclamation-triangle fs-4" style={{ color: '#d32f2f' }}></i>
              </div>
              <div>
                <h4 className="fw-bold mb-0" style={{ fontSize: '1.5rem', color: '#1e293b' }}>
                  {stats.overdue}
          ...
          {filteredItems.length === 0 ? (
            <div className="card border-0 shadow-sm">
              <div className="card-body text-center py-5">
                ...
                <h5 className="fw-semibold text-muted mb-2">No action items found</h5>
                ...
                return (
                  <div key={item.actionItemId} className="col-lg-6 col-xl-4">
                    <div
                      ...
                      style={{
                        transition: 'all 0.2s',
                        borderLeft: `4px solid ${overdueStatus ? '#d32f2f' : item.status === 'Completed' ? '#388e3c' : '#f57c00'}`
                      }}
                      onMouseOver={(e) => {
                    ...
                    >
                      <div className="card-body p-4">
                        ...
                        <div className="d-flex justify-content-between align-items-start mb-3">
                          <div className="flex-grow-1">
                            <div className="d-flex align-items-center gap-2 mb-2">
                              <span className={`badge bg-${priority.color}-subtle text-${priority.color}`}>
                                {priority.label}
                              </span>
                            ...
                            <h6 className="fw-semibold mb-2" style={{ fontSize: '1rem' }}>
                        ...
                        <div className="mb-3 p-2 rounded" style={{ backgroundColor: '#f8f9fa' }}>
                          <small className="text-muted d-flex align-items-center gap-2">
                            <i className="bi bi-calendar-event text-primary"></i>
                            <strong>Meeting:</strong> {item.meetingTitle || 'N/A'}
                          </small>
                        </div>
                        ...
                        <div className="d-flex flex-column gap-2 mb-3">
                          <div className="d-flex align-items-center gap-2 text-muted small">
                            ...
                            <span className={overdueStatus ? 'text-danger fw-semibold' : ''}>
                          ...
                          {item.assignedByEmployeeId && (
                            <div className="d-flex align-items-center gap-2 text-muted small">
                              ...
                              <span>{employeeMap[item.assignedByEmployeeId] || `Employee ${item.assignedByEmployeeId}`}</span>
                            </div>
                          )}
                        </div>
                        ...
                        {item.status === 'Completed' && (
                          <div className="alert alert-success mb-0 d-flex align-items-center gap-2" style={{ padding: '0.5rem 0.75rem' }}>
                            <i className="bi bi-check-circle-fill"></i>
                            <span className="small fw-medium">Task completed successfully</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          >
            <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '12px' }}>
              <div className="modal-header border-0" style={{ padding: '1.5rem' }}>
                <div>
                  <h5 className="modal-title fw-bold mb-1">Mark as Completed</h5>
                  <p className="text-muted small mb-0">Confirm task completion</p>
                </div>
                <button
                  type="button"
                  className="btn-close"
              ...
              <div className="modal-body" style={{ padding: '0 1.5rem 1.5rem' }}>
                ...
                <div className="card bg-light border-0 mb-3">
                  <div className="card-body p-3">
                    <h6 className="fw-semibold mb-3">{selectedItem.taskDescription}</h6>
                    <div className="d-flex flex-column gap-2">
                      <div className="d-flex align-items-center gap-2 text-muted small">
                        <i className="bi bi-calendar-event text-primary"></i>
                        <strong>Meeting:</strong> {selectedItem.meetingTitle || 'N/A'}
                      </div>
                      <div className="d-flex align-items-center gap-2 text-muted small">
                        <i className="bi bi-calendar3 text-danger"></i>
                        <strong>Due Date:</strong> {new Date(selectedItem.dueDate).toLocaleDateString()}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-3">
                  ...
                  <small className="text-muted">
                    These notes will be saved with the action item for future reference
                  </small>
                </div>
                ...
                <div className="alert alert-info d-flex align-items-start gap-2 mb-0">
                  ...
                  <div className="small">
                    This action cannot be undone. Make sure the task is fully completed before confirming.
                  </div>
                </div>
              </div>
