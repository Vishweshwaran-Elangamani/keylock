...
import {
  Users,CheckCircle,AlertCircle,Search,Home,UserCheck,Plus,X,Info,Database,UserX,Check,
} from "lucide-react";
...
const ResourcePoolMapping = () => {
    ...
    try {
      ...
      if (projectsResponse.success && projectsResponse.data) {
       ...
       projectsResponse.data.forEach((project) => {
          if (
            project.projectName && project.projectName.toUpperCase() ===   RESOURCE_POOL_PROJECT_NAME.toUpperCase()
          ) {
          ...
          if (Array.isArray(project.mappedEmployees)) {
            project.mappedEmployees.forEach((emp) => {
              if (emp.roleName?.toLowerCase() === "employee") {
                if (
                  !allMapped.some((m) => m.employeeMasterId === emp.employeeMasterId)
                ) {
                  allMapped.push(emp);
                }
              }
  ...
  const getEmployeeStatus = (empId) => {
    if (isRecentlyMapped(empId) || isEmployeeInResourcePool(empId)) {
      return {
        text: "In Resource Pool",badge: "rp-badge-secondary",icon: CheckCircle,
      };
    }
    if (isEmployeeMappedToAny(empId)) {
      return {
        text: "Mapped to Project", badge: "rp-badge-warning", icon: AlertCircle,
      };
    }
    return {
      text: "Available", badge: "rp-badge-success", icon: Check,
    };
  };

  const filteredEmployees = (activeSearchTerm  ? allEmployees.filter((emp) =>`${emp.firstName} ${emp.lastName} 
  ${emp.roleName} ${emp.departmentName} ${emp.employeeCompanyId}`.toLowerCase() .includes(activeSearchTerm.toLowerCase())
    ) : allEmployees
  ).filter((emp) => {
  ...
  const availableCount = filteredEmployees.filter(
    (emp) => getEmployeeStatus(emp.employeeMasterId).text === "Available").length;

  const handleAddEmployee = (employee) => {
    if (isEmployeeInResourcePool(employee.employeeMasterId)) {
      toast.warning(`${employee.firstName} ${employee.lastName} is already mapped to Resource Pool`, { duration: 3000 }
      );
      return;
    }
  ...
  const employeeIds = selectedEmployees.map((emp) => emp.employeeMasterId);
    ...
    try {
      ...
      const newlyMappedEmployees = selectedEmployees.map((emp) => ({...emp, projectName: RESOURCE_POOL_PROJECT_NAME,}));

      setResourcePoolMappedEmployees((prev) => [...prev,...newlyMappedEmployees.filter(
          (ne) => !prev.some((p) => p.employeeMasterId === ne.employeeMasterId) ),]);
      setMappedEmployees((prev) => [ ...prev, ...newlyMappedEmployees.filter(
         (ne) => !prev.some((p) => p.employeeMasterId === ne.employeeMasterId) ),
      ]);
      setSelectedEmployees([]);
      ...
      <nav aria-label="breadcrumb" className="rp-breadcrumb-nav">
        <ol className="breadcrumb rp-breadcrumb">
          <li className="breadcrumb-item">
            <button type="button" className="rp-breadcrumb-link" onClick={() => navigate("/hr/dashboard/projectmgmt")}
          ...
          <li className="breadcrumb-item">
            <button type="button" className="rp-breadcrumb-link"onClick={() => navigate("/hr/dashboard/projectmgmt/list")} >
              Projects
            </button>
          </li>
      ...
      {resourcePoolProject && (
        <div className="rp-alert-info" role="alert">
          ...
          <div className="rp-alert-content">
            ...
            <span>{resourcePoolProject.projectName}</span> {resourcePoolProject.l2Approver && (
      ...
      {error && (
        <div className="rp-alert-error" role="alert">
          ...
          <div className="rp-alert-error-body">
            ...
            <button type="button"className="rp-error-retry-btn"onClick={fetchInitialData}>
              Retry
            </button>
          </div>
        </div>
        ...
        <div className="rp-col-left">
          <div className="rp-card rp-card-left">
            <div className="rp-card-header">
              ...
              <button type="button" className="rp-btn rp-btn-select-all" onClick={handleSelectAll} disabled={availableCount === 0}>
            ...
            <div className="rp-card-body">
              <div className="rp-search-bar">
                <div className="rp-search-wrapper">
                  ...
                  <input type="text" className="rp-search-input" placeholder="Search by name, department, or ID..."
                    value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyPress={handleSearchKeyPress}/>
                  {searchTerm && (
                    <button type="button" className="rp-clear-btn" onClick={handleCancelSearch}>
                      <X size={14} />
                    </button>)}
                  {activeSearchTerm ? (
                    <button type="button" className="rp-search-btn rp-search-btn-cancel"onClick={handleCancelSearch}>
                      <X size={14} />
                      <span>Cancel</span>
                    </button>
                  ) : (
                    <button type="button"  className="rp-search-btn" onClick={handleSearch}>
                      <Search size={14} />
                      <span>Search</span>
                    </button>
                  )}
                      ...
                      <tr>
                        <td colSpan="5" className="rp-empty-state">
                          ...
                          <p className="rp-empty-text">
                            {activeSearchTerm ? "No matches for your search." : "All eligible employees are mapped."}
                          </p>
                        </td>
                      </tr>
                    ) : (
                        ...
                        return (
                          <tr key={emp.employeeMasterId}>
                            <td className="rp-cell-employee">
                              ...
                              <span className="rp-employee-id">{emp.employeeCompanyId}</span>
                              {emp.email && (
                                <span className="rp-employee-email"> {emp.email}</span>
                              )}
                            </td>
                            <td>
                            ...
                            <td>
                              <span className="rp-department">{emp.departmentName} </span>
                            </td>
                            <td>
                            ...
                            <td>
                              <button type="button" className={`rp-emp-action-btn ${
                                  isSelected ? "rp-emp-action-btn--selected" : inResourcePool
                                    ? "rp-emp-action-btn--in-pool": status.text === "Available"
                                    ? "rp-emp-action-btn--add": "rp-emp-action-btn--warn"
                                }`}
                                onClick={() => handleAddEmployee(emp)} disabled={isSelected || inResourcePool}
                                title={ inResourcePool? "Already in Resource Pool" : isSelected
                                ? "Selected": "Add to selection"
                                }>
                                {isSelected ? (
                                  <Check size={16} />
                                ) : inResourcePool ? (
          ...
          <div className="rp-card rp-card-right">
            <div className="rp-card-header">
              ...
              {selectedEmployees.length > 0 && (
                <button type="button" className="rp-btn rp-btn-clear"onClick={handleClearAll}>
                  <X size={14} />
                </button>
              )}
            </div>
            <div className="rp-card-body rp-selected-body">
              <div className="rp-selected-list">
                {selectedEmployees.length === 0 ? (
                  <div className="rp-selected-empty">
                    ...
                    <span className="rp-empty-text"> Select available employees from the left</span>
                  </div>
                ) : (
                  selectedEmployees.map((emp, index) => (
                    <div key={emp.employeeMasterId} className="rp-selected-item">
                      ...
                      <button type="button" className="rp-selected-remove-btn" onClick={() =>
              ...
              <div className="rp-selected-footer">
                <button type="button" className="rp-btn-map" onClick={handleMapToResourcePool}
                    disabled={selectedEmployees.length === 0 || isSubmitting}>
                  {isSubmitting ? (
                    <>
                      <span className="rp-inline-spinner" />
                      <span> Mapping {selectedEmployees.length} employee(s)...</span>
                    </>
                  ) : (
                    <>
                      <UserCheck size={18} />
        ...
        <div className="rp-modal-overlay" onClick={() => setShowClearModal(false)}>
          <div className="rp-modal-container"onClick={(e) => e.stopPropagation()}>
            <div className="rp-modal-header">
              ...
              <button type="button"  className="rp-modal-close" onClick={() => setShowClearModal(false)}>× </button>
            </div>
            ...
            <div className="rp-modal-footer">
              <button type="button" className="rp-modal-btn rp-modal-btn--secondary"onClick={() => setShowClearModal(false)}>
                   Cancel
              </button>
              <button type="button"  className="rp-modal-btn rp-btn-clear-confirm" onClick={confirmClearAll}>
                <X size={18} />
                <span>Clear All</span>
              </button>
            </div>
        ...
        <div className="rp-modal-overlay" onClick={() => !isSubmitting && setShowMapModal(false)}>
          <div className="rp-modal-container" onClick={(e) => e.stopPropagation()}>
            <div className="rp-modal-header">
              ...
              <button type="button" className="rp-modal-close" onClick={() => setShowMapModal(false)} disabled={isSubmitting}>
                ×
              </button>
            </div>
            <div className="rp-modal-body">
              ...
              <div className="rp-modal-actions-list">
                ...
                <ul>
                  ...
                  <li>Set Resource Pool&apos;s L2 Approver as their reportingmanager</li>
              ...
              <div className="rp-modal-selected-list">
                ...
                <div className="rp-modal-employees">
                  {selectedEmployees.map((emp) => (
                    <span key={emp.employeeMasterId} className="rp-modal-employee-badge"> {emp.firstName} {emp.lastName}</span>
                  ))}
                </div>
              </div>
            </div>
            <div className="rp-modal-footer">
              <button type="button" className="rp-modal-btn rp-modal-btn--secondary" onClick={() => setShowMapModal(false)} disabled={isSubmitting}>
                Cancel
              </button>
              <button type="button" className="rp-modal-btn rp-btn-map-confirm" onClick={confirmMapToResourcePool} disabled={isSubmitting} >
                {isSubmitting ? (
                  <>
                    <span className="rp-inline-spinner" />
                    <span>Mapping...</span>
