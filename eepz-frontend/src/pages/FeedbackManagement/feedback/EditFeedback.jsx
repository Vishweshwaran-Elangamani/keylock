// src/pages/feedback/EditFeedback.jsx
...
export default function EditFeedback() {
    ...
    const fetchItem = async () => {
      ...
      try {
        if (type === 'mentor') {
          ...
          setForm({ rating: item.rating || 4, feedbackComments: item.feedbackComments || '', feedbackContent: '' });
        } else if (type === 'peer') {
          const res = await peerQueueApi.getById(id);
          const item = res.data?.data;
          setForm({ rating: 0, feedbackComments: '', feedbackContent: item.feedbackContent || '' });
        } else {
  ...
  const submit = async (e) => {
    ...
    try {
      if (type === 'mentor') {
        await mentorFeedbackApi.update(id, { rating: Number(form.rating), feedbackComments: form.feedbackComments });
      } else {
        // Peer queue doesn't have an update endpoint in your spec; skip or show error
        setError('Peer feedback cannot be edited after submission (no API endpoint).');
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="container-fluid py-3" style={{ maxWidth: '900px' }}>
      <div className="d-flex justify-content-between align-items-start mb-4">
        <div>
          <h2 className="fw-bold mb-1" style={{ color: 'var(--color-primary-1)' }}>✏️ Edit Feedback</h2>
          <p className="mb-0 small" style={{ color: 'var(--muted)' }}>Update your submitted feedback</p>
        </div>
            ...
            {type === 'mentor' && (
              <>
                <div className="col-md-6">
                  ...
                  />
                  <div className="small text-muted">Selected: {form.rating} ⭐</div>
                </div>
                <div className="col-12">
                  <label className="form-label small">Feedback Comments *</label>
                  <textarea
            ...
            {type === 'peer' && (
              <div className="col-12">
                ...
                <textarea
                  ...
                  onChange={(e) => setForm({ ...form, feedbackContent: e.target.value })}
                  placeholder="Update your feedback…"
                  disabled
                />
                <small className="text-muted d-block mt-1">
                  ⚠️ Peer feedback cannot be edited after submission (API limitation).
                </small>
              </div>
            )}
            <div className="col-12 d-grid">
              <button className="btn btn-primary" disabled={loading || type === 'peer'}>
