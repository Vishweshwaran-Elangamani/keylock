// src/pages/feedback/SearchFeedback.jsx
import React, { useEffect, useState } from 'react';
import { Search, RefreshCw, AlertTriangle } from 'lucide-react';
import { managerReviewApi, mentorFeedbackApi, peerQueueApi, orgGoalFeedbackApi } from '../../../services/feedbackmanagement/feedbackApi';
...
export default function SearchFeedback() {
  ...
  const [allData, setAllData] = useState({ manager: [], mentor: [], peer: [], orgGoal: [] });
  ...
  return (
    <div className="container-fluid py-3" style={{ maxWidth: '1000px' }}>
      <div className="d-flex justify-content-between align-items-start mb-4">
        <div>
          <h2 className="fw-bold mb-1" style={{ color: 'var(--color-primary-1)' }}>🔍 Search Feedback</h2>
          <p className="mb-0 small" style={{ color: 'var(--muted)' }}>Search across all feedback types</p>
        </div>
        <button
      ...
      <div className="card border-0" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow)' }}>
        <div className="card-body">
          ...
          {results.length === 0 ? (
            <p className="text-muted mb-0">No results found. Try searching with different keywords.</p>
          ) : (
                  ...
                  <div className="card h-100 border-0" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)' }}>
                    <div className="card-body">
                      <div className="d-flex justify-content-between align-items-start mb-2">
                        ...
                        {item.status && <Badge text={item.status} color="#24A148" />}
                      </div>
