...
import { RefreshCw, AlertTriangle, Eye, Filter, Clock, CheckCircle, User, ArrowLeft, Send } from 'lucide-react';
...
export default function DeptHeadTeamSubmissions() {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      try {
        ...
        if (mentorRes.data?.success && Array.isArray(mentorRes.data.data)) {
          mentorData = mentorRes.data.data.map(m => ({
            ...
            submittedBy: employeeMap[m.mentorEmployeeId] || `Mentor ${m.mentorEmployeeId}`,
      ...
      try {
        ...
        if (peerRes.data?.success && Array.isArray(peerRes.data.data)) {
          peerData = peerRes.data.data.map(p => ({
            ...p,
            type: 'Peer Feedback',
            submittedBy: employeeMap[p.submittedByEmployeeId] || `Employee ${p.submittedByEmployeeId}`,
      ...
      let hrData = [];
      try {
        const hrRes = await axios.get(`${API_BASE}/HrFeedbackForm/responses/all`);
        if (hrRes.data?.success && Array.isArray(hrRes.data.data)) {
          hrData = hrRes.data.data.map(h => ({
            ...h,
            type: 'HR Form',
            submittedBy: employeeMap[h.employeeId] || `Employee ${h.employeeId}`,
    ...
    <div className="container-fluid py-3" style={{ maxWidth: '1400px' }}>
      ...
      <div className="d-flex justify-content-between align-items-start mb-4">
        <div className="d-flex gap-2 align-items-start">
          ...
          <div>
            <h2 className="fw-bold mb-1" style={{ color: 'var(--color-primary-1)' }}>Team Submissions</h2>
            <p className="mb-0 small text-muted">View all feedback submissions from team</p>
          </div>
        </div>
        <button
      ...
      <div className="row g-3 mb-4">
        ...
        <div className="col-6 col-md-3">
          <div className="card border-0" style={{ border: '1px solid var(--border)' }}>
            <div className="card-body">
              <h4 className="fw-bold" style={{ color: '#0F62FE' }}>{stats.mentor}</h4>
        ...
        <div className="col-6 col-md-3">
          <div className="card border-0" style={{ border: '1px solid var(--border)' }}>
            <div className="card-body">
              <h4 className="fw-bold" style={{ color: '#24A148' }}>{stats.peer}</h4>
        ...
        <div className="col-6 col-md-3">
          <div className="card border-0" style={{ border: '1px solid var(--border)' }}>
            <div className="card-body">
              <h4 className="fw-bold" style={{ color: '#E2B93B' }}>{stats.hr}</h4>
        ...
        <div className="card-body">
          <div className="row g-3">
            <div className="col-md-6">
              <label className="form-label fw-bold small">Filter by Type:</label>
              <select
              ...
              <table className="table table-hover mb-0">
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--border)' }}>
                    <th style={{ color: 'var(--color-primary-1)' }}>Submitted By</th>
                ...
                <tbody>
                  {filteredSubmissions.map((submission, idx) => {
                    const statusColor = submission.status === 'Approved' ? '#24A148' : submission.status === 'Pending' ? '#E2B93B' : '#0F62FE';
                    const typeColor = submission.type === 'Mentor Feedback' ? '#0F62FE' : submission.type === 'Peer Feedback' ? '#24A148' : '#E2B93B';
                    
                    return (
                      <tr key={`${submission.type}-${idx}`}>
                        <td className="small fw-bold">{submission.submittedBy}</td>
                        <td>
                          <Badge text={submission.type} color={typeColor} />
                        </td>
                        <td>
                          <Badge text={submission.status || 'Submitted'} color={statusColor} />
                        </td>
