...
import {
  ...
  Send, Search, Eye, User, Zap, Star, Users, Award, MessageSquare
} from 'lucide-react';
import { Link } from 'react-router-dom';
import axios from 'axios';
import { peerQueueApi, smeApi } from '../../services/feedbackmanagement/feedbackApi';
  ...
  const checkIfMentor = async () => {
    try {
      ...
      if (smeResponse.data?.success && Array.isArray(smeResponse.data.data)) {
        const isSme = smeResponse.data.data.some(sme => sme.employeeId === empId);
        setIsMentor(isSme);
       
        if (isSme) {
          try {
            const feedbackResponse = await axios.get(`${API_BASE}/mentorfeedback/about-me/${empId}`);
            if (feedbackResponse.data?.success && Array.isArray(feedbackResponse.data.data)) {
