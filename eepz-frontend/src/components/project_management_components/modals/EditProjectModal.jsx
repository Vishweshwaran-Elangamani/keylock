...
const CustomCalendar = ({ value, onChange, onClose, minDate }) => {
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      const classNames = [
        "epm-cal-day",
        isToday && "epm-cal-day--today",
        isSelected && "epm-cal-day--selected",
        isDisabled && "epm-cal-day--disabled",
      ]
        .filter(Boolean)
        .join(" ");
    ...
    for (let day = 1; day <= remainingDays; day++) {
      days.push(
        <div key={`next-${day}`} className="epm-cal-day epm-cal-day--outside">
          {day}
        </div>
      );
    }
  ...
  return (
    <div className="epm-cal-root">
      <div className="epm-cal-header">
        <button
          type="button"
          className="epm-cal-nav-btn"
          onClick={handlePrevMonth}
        >
          <ChevronLeft size={14} />
        </button>
        <span className="epm-cal-month">
      ...
      <div className="epm-cal-weekdays">
        {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
          <div key={d} className="epm-cal-weekday">
            {d}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="epm-dd-wrapper" ref={dropdownRef}>
      <label className="epm-label">
        {label}
        {required && <span className="epm-label-required">*</span>}
      </label>
      <div className="epm-dd-inner">
        <button
          type="button"
          ref={buttonRef}
          className={`epm-dd-button ${isOpen ? "epm-dd-button--open" : ""}`}
          onClick={() => setIsOpen(!isOpen)}
        >
          <span
            className={`epm-dd-value ${
              value ? "epm-dd-value--filled" : "epm-dd-value--placeholder"
            }`}
          >
            {getDisplayValue()}
          </span>
            ...
            >
              {options.map((opt, idx) => {
                ...
                return (
                  <li
                    key={idx}
                    className={`epm-dd-item ${
                      selected ? "epm-dd-item--selected" : ""
                    }`}
            ...
            <div className="epm-date-popup-inner">
              ...
              <div className="epm-date-grid">
                {cells.map((c, idx) => {
                  ...
                  const classes = [
                    "epm-date-cell",
                    !c.current && "epm-date-cell--outside",
                    isToday && "epm-date-cell--today",
                    isSelected && "epm-date-cell--selected",
                  ];
