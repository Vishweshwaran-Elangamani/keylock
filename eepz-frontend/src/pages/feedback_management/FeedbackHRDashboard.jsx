...
const StatCard = ({ label, value, Icon, color }) => (
  <div style={{ 
    ...
    <h3 className="fw-bold mb-1" style={{ color, fontSize: '1.75rem' }}>{value}</h3>
    <p className="mb-0" style={{ fontSize: '0.813rem', color: '#6c757d' }}>{label}</p>
  </div>
);
...
export default function FeedbackHRDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: 'Total Feedback', value: feedback.length, Icon: FileText, color: '#97247E' },
    ];
        ...
        <div style={{ 
          ...
          <ul className="nav nav-tabs border-0 mb-0" style={{ gap: '0.5rem' }}>
            <li className="nav-item">
              <button
                className={`nav-link border-0 ${activeTab === 'overview' ? 'active' : ''}`}
                onClick={() => setActiveTab('overview')}
                style={{
                  color: activeTab === 'overview' ? '#97247E' : '#6c757d',
                  borderBottom: activeTab === 'overview' ? '3px solid #97247E' : '3px solid transparent',
                  fontWeight: 600,
            ...
            <li className="nav-item">
              <button
                className={`nav-link border-0 ${activeTab === 'hr-operations' ? 'active' : ''}`}
                onClick={() => setActiveTab('hr-operations')}
                style={{
                  color: activeTab === 'hr-operations' ? '#97247E' : '#6c757d',
                  borderBottom: activeTab === 'hr-operations' ? '3px solid #97247E' : '3px solid transparent',
                  fontWeight: 600,
                  background: 'transparent',
                  padding: '0.75rem 1rem'
                }}
            ...
            <li className="nav-item">
              <button
                className={`nav-link border-0 ${activeTab === 'peer-feedback' ? 'active' : ''}`}
                onClick={() => setActiveTab('peer-feedback')}
                style={{
                  color: activeTab === 'peer-feedback' ? '#97247E' : '#6c757d',
                  borderBottom: activeTab === 'peer-feedback' ? '3px solid #97247E' : '3px solid transparent',
                  fontWeight: 600,
                  background: 'transparent',
                  padding: '0.75rem 1rem'
                }}
        ...
        {activeTab === 'overview' && (
          <div style={{ 
            ...
            <div className="mb-4">
              ...
              {/* HR-specific actions */}
              <h6 className="small text-muted mb-2" style={{textAlign:'left'}}>HR Functions</h6>
              ...
              <h6 className="small text-muted mb-2" style={{textAlign:'left'}}>Submit Feedback</h6>
          ...
          <div style={{ 
            ...
            {feedback.length === 0 ? (
              <div className="text-center py-5">
                <FileText size={48} className="mb-3" style={{ color: '#cbd5e1' }} />
                ...
                <table className="table table-hover mb-0">
                  <thead>
                    <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
                      <th style={{ color: '#97247E', fontSize: '0.813rem', fontWeight: 700, textTransform: 'uppercase', padding: '1rem' }}>From</th>
                      <th style={{ color: '#97247E', fontSize: '0.813rem', fontWeight: 700, textTransform: 'uppercase', padding: '1rem' }}>To</th>
                      <th style={{ color: '#97247E', fontSize: '0.813rem', fontWeight: 700, textTransform: 'uppercase', padding: '1rem' }}>Content</th>
                      <th style={{ color: '#97247E', fontSize: '0.813rem', fontWeight: 700, textTransform: 'uppercase', padding: '1rem' }}>Date</th>
                      <th style={{ color: '#97247E', fontSize: '0.813rem', fontWeight: 700, textTransform: 'uppercase', padding: '1rem' }}>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {feedback.map((item, index) => (
                      <tr key={`feedback-${item.queueId || index}`} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '1rem', fontSize: '0.875rem', fontWeight: 600 }}>{item.submitterName}</td>
                        <td style={{ padding: '1rem', fontSize: '0.875rem', fontWeight: 600 }}>{item.recipientName}</td>
                        <td style={{ padding: '1rem', fontSize: '0.875rem', maxWidth: '300px' }} className="text-truncate">
