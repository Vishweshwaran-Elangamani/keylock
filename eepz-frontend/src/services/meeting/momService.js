...
const momService = {
  ...
  createMom: async (momData) => {
    ...
    if (meetingDate < minDate || meetingDate > maxDate) {
      throw {
        success: false,
        message: `Meeting date must be between ${minDate.toISOString().split('T')[0]} and ${maxDate.toISOString().split('T')[0]}`,
  ...
  updateMom: async (momData) => {
    ...
    if (!hasData) {
      throw {
        success: false,
        message: 'Update payload cannot be empty. At least one field must be provided.',
  ...
  deleteMom: async (momId, confirmed = false) => {
    ...
    return apiRequest("delete", `/Mom/${momId}?confirmationToken=${confirmationToken}`);
  },
  ...
  deleteMomWithConfirmation: async (momId, meetingTitle = '') => {
    ...
    try {
      const confirmationToken = `DELETE_${momId}`;
      const result = await apiRequest("delete", `/Mom/${momId}?confirmationToken=${confirmationToken}`);
  ...
  validation: {
    ...
    validateMeetingTitle: (title) => {
      if (!title || title.trim().length < 3) {
        return { valid: false, message: 'Meeting title must be at least 3 characters long' };
      }
      if (title.length > 200) {
        return { valid: false, message: 'Meeting title cannot exceed 200 characters' };
      }
      return { valid: true };
    },
    ...
    validateDiscussionPoint: (pointText) => {
      if (!pointText || pointText.trim().length === 0) {
        return { valid: false, message: 'Discussion point text cannot be empty' };
      }
      if (pointText.length > 2000) {
        return { valid: false, message: 'Discussion point text cannot exceed 2000 characters' };
      }
      return { valid: true };
    },
    ...
    validateActionItem: (item) => {
      if (!item.taskDescription || item.taskDescription.trim().length === 0) {
        return { valid: false, message: 'Action item description cannot be empty' };
      }
      if (item.taskDescription.length > 1000) {
        return { valid: false, message: 'Action item description cannot exceed 1000 characters' };
      }
      ...
      if (dueDate < yesterday) {
        return { valid: false, message: 'Action item due date cannot be in the past' };
      }
