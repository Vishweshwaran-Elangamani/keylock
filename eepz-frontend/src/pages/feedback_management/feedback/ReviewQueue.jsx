// src/pages/feedback/ReviewQueue.jsx
import React, { useEffect, useMemo, useState } from 'react';
import { RefreshCw, AlertTriangle, ThumbsUp, ThumbsDown, CheckCircle } from 'lucide-react';
...
export default function ReviewQueue() {
  const user = useMemo(() => JSON.parse(localStorage.getItem('user') || '{}') || { empId: 1001, name: 'Alice HR' }, []);
  ...
  const fetchPending = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err.message || 'Failed to fetch pending queue');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const approve = async (item) => {
    ...
    try {
      ...
      setSuccessMsg(`Approved feedback from ${item.submitterName || item.submittedByEmployeeId}`);
      fetchPending();
    } catch (err) {
  ...
  const reject = async (item) => {
    ...
    try {
      ...
      setSuccessMsg(`Rejected feedback from ${item.submitterName || item.submittedByEmployeeId}`);
      fetchPending();
    } catch (err) {
  ...
  return (
    <div className="container-fluid py-3" style={{ maxWidth: '1200px' }}>
      <div className="d-flex justify-content-between align-items-start mb-4">
        <div>
          <h2 className="fw-bold mb-1" style={{ color: 'var(--color-primary-1)' }}>✅ Review Queue (HR)</h2>
          <p className="mb-0 small" style={{ color: 'var(--muted)' }}>Approve or reject pending peer feedback</p>
        </div>
        <button
      ...
      {successMsg && (
        <div className="alert alert-success d-flex align-items-center gap-2" style={{ borderRadius: 'var(--radius-md)' }}>
          <CheckCircle size={18} />
      ...
      <div className="card border-0" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow)' }}>
        <div className="card-body">
          ...
          {pending.length === 0 ? (
            <p className="text-muted mb-0">No pending peer feedback to review</p>
          ) : (
                  ...
                  <div className="card h-100 border-0" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)' }}>
                    <div className="card-body">
                      <div className="d-flex justify-content-between align-items-start mb-2">
                        <h6 className="mb-0">
                          {item.submitterName || item.submittedByEmployeeId} → {item.recipientName || item.recipientEmployeeId}
                        </h6>
                        <Badge text={item.status || 'Pending'} color="#0F62FE" />
                      </div>
                      <p className="small mb-2">{item.feedbackContent}</p>
                      {item.isAnonymous && (
                        <small className="text-muted d-block mb-2">🔒 Anonymous</small>
                      )}
                      <small className="text-muted d-block mb-3">
                        Submitted: {item.createdAt ? new Date(item.createdAt).toLocaleDateString() : '—'}
                      </small>
                      <div className="d-grid gap-2">
                        <button className="btn btn-sm btn-outline-success" onClick={() => approve(item)}>
                          <ThumbsUp size={14} className="me-1" />
                          Approve
                        </button>
                        <button className="btn btn-sm btn-outline-danger" onClick={() => reject(item)}>
