...
import { useState, useEffect } from "react";
import { toast } from "sonner";
import { Modal, Button, Form, Spinner, Badge, Tabs, Tab } from "react-bootstrap";
import violationService from "../../../../services/hr_operations/hr/violationService";
import "../../../../styles/hr_operations/hr/complianceIssues.css";
...
const ComplianceIssues = () => {
  ...
  const applyFilters = () => {
    ...
    if (filters.searchTerm.trim() !== "") {
      ...
      filtered = filtered.filter(
        (v) =>
          (v.employeeName && v.employeeName.toLowerCase().includes(searchLower)) ||
          (v.employeeEmail && v.employeeEmail.toLowerCase().includes(searchLower)) ||
          (v.violationType && v.violationType.toLowerCase().includes(searchLower)) ||
          (v.policyName && v.policyName.toLowerCase().includes(searchLower))
      );
    }
  ...
  const applyEscalationFilters = () => {
    ...
    if (escalationFilters.status !== "All") {
      filtered = filtered.filter((e) => e.escalationStatus === escalationFilters.status);
    }

    if (escalationFilters.level !== "All") {
      filtered = filtered.filter((e) => e.escalationLevel === escalationFilters.level);
    }
    ...
    <div className="ci-root">
      ...
      <div className="ci-stats-row">
        <div className="ci-stat-card stat-total">
          ...
          <div className="ci-stat-content">
            <div className="ci-stat-label">Total Violations</div>
            <div className="ci-stat-value">{displayStats.totalViolations || 0}</div>
          </div>
        </div>
        ...
        <div className="ci-stat-card stat-active">
          ...
          <div className="ci-stat-content">
            <div className="ci-stat-label">Active Cases</div>
            <div className="ci-stat-value">{displayStats.activeViolations || 0}</div>
          </div>
        </div>
        ...
        <div className="ci-stat-card stat-resolved">
          ...
          <div className="ci-stat-content">
            <div className="ci-stat-label">Resolved</div>
            <div className="ci-stat-value">{displayStats.resolvedViolations || 0}</div>
          </div>
        </div>
        ...
        <div className="ci-stat-card stat-escalations">
          ...
          <div className="ci-stat-content">
            ...
            <div className="ci-stat-value">{displayStats.totalEscalations || 0}</div>
          </div>
        </div>
      </div>
                ...
                <div key={violation.violationId} className="ci-violation-card">
                  <div className="ci-violation-header">
                    <div>
                      <h5 className="ci-employee-name">
                        {violation.employeeName || "Unknown Employee"}
                        <Badge bg={getSeverityBadge(violation.severity)} className="ms-2">
                          {violation.severity} Severity
                        </Badge>
                      </h5>
                      <p className="ci-employee-id">
                        User ID: {violation.employeeUserId} • {violation.employeeEmail || "N/A"}
                      </p>
                    </div>
                    <Badge bg={getStatusBadge(violation.status)} className="ci-status-badge">
                      {getStatusLabel(violation.status)}
                    </Badge>
                  </div>
                  ...
                  <div className="ci-violation-details">
                    <div className="ci-detail-row">
                      <span className="ci-detail-label">Violation Type</span>
                      <span className="ci-detail-value">{violation.violationType || "N/A"}</span>
                    </div>
                    <div className="ci-detail-row">
                      <span className="ci-detail-label">Policy</span>
                      <span className="ci-detail-value">{violation.policyName || "N/A"}</span>
                    </div>
                    <div className="ci-detail-row">
                      <span className="ci-detail-label">Date Reported</span>
                      <span className="ci-detail-value">
            ...
            <div className="ci-filters-header">
              ...
              {(escalationFilters.searchTerm ||
                ...
                <button
                  className="ci-btn-clear-link"
                  onClick={() =>
                    setEscalationFilters({ status: "All", level: "All", searchTerm: "" })
                  }
                >
                  Reset all filters
                </button>
            ...
            <div className="ci-filters-row">
              <div className="ci-filter-group ci-filter-search">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEscalationFilters({ ...escalationFilters, searchTerm: e.target.value })
                  }
                />
              </div>
              ...
              <div className="ci-filter-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEscalationFilters({ ...escalationFilters, status: e.target.value })
                  }
                >
                  <option value="All">All Status</option>
              ...
              <div className="ci-filter-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEscalationFilters({ ...escalationFilters, level: e.target.value })
                  }
                >
        ...
        <Modal.Body>
          {selectedViolation && (
            <div className="ci-detail-grid">
              ...
              <div className="ci-detail-item">
                <label>Date Reported:</label>
                <span>{new Date(selectedViolation.reportedDate).toLocaleDateString()}</span>
              </div>
              <div className="ci-detail-item">
                <label>Reported By:</label>
                <span>{selectedViolation.reportedByEmail || "N/A"}</span>
          ...
          {selectedEscalation && (
            <div className="ci-detail-grid">
              ...
              <div className="ci-detail-item">
                ...
                <span className="text-danger fw-bold">{selectedEscalation.daysOverdue} days</span>
              </div>
              <div className="ci-detail-item">
                <label>SLA Deadline:</label>
                <span>{new Date(selectedEscalation.slaDeadline).toLocaleDateString()}</span>
              </div>
              <div className="ci-detail-item">
                <label>Escalated To:</label>
              ...
              <div className="ci-detail-item">
                ...
                <span>{new Date(selectedEscalation.submittedAt).toLocaleString()}</span>
              </div>
              <div className="ci-detail-item">
              ...
              {selectedEscalation.resolvedAt && (
                <>
                  <div className="ci-detail-item">
                    <label>Resolved At:</label>
                    <span>{new Date(selectedEscalation.resolvedAt).toLocaleString()}</span>
                  </div>
                  <div className="ci-detail-item">
