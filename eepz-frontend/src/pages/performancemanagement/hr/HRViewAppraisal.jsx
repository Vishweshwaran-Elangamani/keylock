    ...
    async function fetchAppraisals() {
      try {
        ...
        if (response.data?.success) {
          ...
          const initiatedActions = new Set([
            "send",
            "sent",
            "initiate",
            "initiated",
          ...
          const nonDraftStatuses = new Set([
            "completed",
            "in review",
            "in-progress",
            "in progress",
          ...
          const normalizeAppraisal = (appraisal) => {
            const comps =
              appraisal.competencies || appraisal.Competencies || [];
            const normalizedComps = (Array.isArray(comps) ? comps : []).map(
              (comp) => ({
                competencyName:
                  comp.competencyName ||
                  comp.CompetencyName ||
                  (comp.Competency && comp.Competency.Name) ||
                  "Unknown",
                employeeRating:
                  comp.employeeRating ?? comp.EmployeeRating ?? null,
                employeeComments:
                  comp.employeeComments ?? comp.EmployeeComments ?? "",
                l1ReviewerName:
                  comp.l1ReviewerName || comp.L1ReviewerName || "-",
                l1Rating: comp.l1Rating ?? comp.L1Rating ?? null,
                l1Comments: comp.l1Comments ?? comp.L1Comments ?? "",
                l1ReviewStatus:
                  comp.l1ReviewStatus || comp.L1ReviewStatus || "",
                l2ReviewerName:
                  comp.l2ReviewerName || comp.L2ReviewerName || "-",
                l2Rating: comp.l2Rating ?? comp.L2Rating ?? null,
                l2Comments: comp.l2Comments ?? comp.L2Comments ?? "",
                l2ReviewStatus:
                  comp.l2ReviewStatus || comp.L2ReviewStatus || "",
          ...
          const initiatedAppraisals = raw.filter((appraisal) => {
            ...
            if (!Array.isArray(comps) || comps.length === 0) {
              console.log(
                `Employee: ${
                  appraisal.employeeName || appraisal.EmployeeName || "unknown"
                } - no competencies array`
              );
            ...
            for (const comp of comps) {
              const action = normalize(
                comp.action || comp.Action || comp.ActionType || comp.actionType
              );
              const status = normalize(
                comp.status || comp.Status || comp.currentStatus || comp.state
              );

              if (action && initiatedActions.has(action)) hasInitiated = true;
              if (
                status &&
                (nonDraftStatuses.has(status) ||
                  (status !== "" &&
                    status !== "draft" &&
            ...
            console.log(
              `Employee: ${
                ...
                appraisal.EmployeeName ||
                "unknown"
              }, hasInitiated=${hasInitiated}, hasNonDraftStatus=${hasNonDraftStatus}`
            );
          ...
          if (initiatedAppraisals.length === 0 && raw.length > 0) {
            console.warn(
              "No appraisals passed the 'initiated' filter. Showing all fetched appraisals as a fallback."
            );
    ...
    <>
      ...
      >
        <div className="container pt-4">
          ...
          <div className="row mb-4">
            <div className="col-md-6 col-12 mb-2">
              <div
                ...
                style={{
                  background: "#f7faf8",
                  borderLeft: "5px solid var(--badge-green)",
                }}
              >
                <div className="card-body d-flex align-items-center">
                  <span
                    className="badge badge-completed me-3"
                    style={{ fontSize: "1.3rem", minWidth: 52 }}
                  >
                    {completedCount}
                  </span>
                  <span
                    className="fw-bold"
                    style={{ color: "var(--eepz-dark-purple)", fontSize: 17 }}
                  >
                    Completed
                  </span>
                </div>
            ...
            <div className="col-md-6 col-12 mb-2">
              <div
                ...
                style={{
                  background: "#fffbe6",
                  borderLeft: "5px solid var(--badge-yellow)",
                }}
              >
                <div className="card-body d-flex align-items-center">
                  <span
                    className="badge badge-pending me-3"
                    style={{ fontSize: "1.3rem", minWidth: 52 }}
                  >
                    {pendingCount}
                  </span>
          ...
          <table
            className="table eepz-table table-bordered table-striped align-middle"
            style={{ borderRadius: 16, overflow: "hidden", minWidth: 850 }}
          >
                ...
                <tr>
                  <td colSpan={9} className="text-center py-4">
                    ...
                    <span style={{ color: "#6B7280" }}>
                      No initiated appraisals match your search
                    </span>
                  </td>
                </tr>
              ) : (
                                ...
                                <div className="card h-100">
                                  <div className="card-body p-3">
                                    ...
                                    <div>
                                      ...
                                      <span>{c.employeeRating ?? "-"}</span>
                                      <br />
                                      <span className="fw-semibold">
                                        Employee Comments:
                                      </span>{" "}
                                      <span>{c.employeeComments ?? "-"}</span>
                                      <hr className="my-2" />
                                      <span className="fw-semibold">
                                        L1 Reviewer:
                                      </span>{" "}
                                      <span>{c.l1ReviewerName ?? "-"}</span>
                                      <br />
                                      <span className="fw-semibold">
                                        L1 Rating:
                                      </span>{" "}
                                      <span>{c.l1Rating ?? "-"}</span>
                                      <br />
                                      <span className="fw-semibold">
                                        L1 Comments:
                                      </span>{" "}
                                      <span>{c.l1Comments ?? "-"}</span>
                                      <hr className="my-2" />
                                      <span className="fw-semibold">
                                        L2 Reviewer:
                                      </span>{" "}
                                      <span>{c.l2ReviewerName ?? "-"}</span>
                                      <br />
                                      <span className="fw-semibold">
                                        L2 Rating:
                                      </span>{" "}
                                      <span>{c.l2Rating ?? "-"}</span>
                                      <br />
                                      <span className="fw-semibold">
                                        L2 Comments:
                                      </span>{" "}
                                      <span>{c.l2Comments ?? "-"}</span>
                                      <hr className="my-2" />
                                      <span className="fw-semibold">
                                        Status:
                                      </span>{" "}
