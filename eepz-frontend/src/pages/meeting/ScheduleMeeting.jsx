...
import {
  ...
  FileText,Search,Check, X,Plus, Home,
...
const ScheduleMeeting = () => {
  ...
  const [formData, setFormData] = useState({
    meetingType: "One-on-One", meetingTitle: "", participantEmployeeIds: [],
    meetingDate: "",meetingTime: "",duration: "1",meetingLink: "",agenda: "",sendCalendarInvite: true,reminder: 1,
});
        ...
        try {
          ...
          if (response.success) {
            const items = response.data.items.map((item) => ({
              employeeId: item.employeeId,firstName: item.employeeName,
              lastName: "", email: item.email, roleName: "", departmentName: item.departmentName,}));
  ...
  const handleOneOnOneChange = (empId) => {
    setFormData((prev) => ({ ...prev, participantEmployeeIds: [empId],
    }));
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (
      !formData.meetingTitle || !formData.meetingDate ||
      !formData.meetingTime ||formData.participantEmployeeIds.length === 0
    ) {
      toastr.error(
    ...
    try {
      setLoading(true);
      const payload = {
        meetingTitle: formData.meetingTitle,meetingType: formData.meetingType,
        meetingDate: `${formData.meetingDate}T${formData.meetingTime}:00`,meetingLink: formData.meetingLink,
        agenda: formData.agenda,participantEmployeeIds: formData.participantEmployeeIds,
        duration: formData.duration, sendCalendarInvite: formData.sendCalendarInvite, reminder: formData.reminder,
      };
  ...
  const meetingTypeOptions = [
    "One-on-One", "Team Meeting", "Presentation", "Other",];

  const durationOptions = [
    { value: "0.5", label: "30 minutes" },{ value: "1", label: "1 hour" },
    { value: "1.5", label: "1.5 hours" },{ value: "2", label: "2 hours" },
  ...
  const ensureCalendarMonthYear = () => {
    if (calendarMonth === null || calendarYear === null) {
      const base = formData.meetingDate ? new Date(formData.meetingDate) : new Date();
        ...
        <div className="col-lg-10 col-xl-9">
          <nav aria-label="breadcrumb" className="sched-breadcrumb-nav">
            <ol className="breadcrumb mb-0 d-flex align-items-center sched-breadcrumb">
              <li className="breadcrumb-item d-flex align-items-center">
                <button onClick={() => navigate("/manager/dashboard/")} className="sched-breadcrumb-link"
                  style={{ display: "flex", alignItems: "center", padding: 0 }}>
                  <Home size={18} />
                </button>
              </li>
              <li className="sched-breadcrumb-separator"style={{ margin: "0 4px" }}> /</li>
              <li className="breadcrumb-item d-flex align-items-center">
                <button onClick={() => navigate("/manager/dashboard/meetmom")}
                  className="sched-breadcrumb-link"style={{ padding: 0, marginLeft: "2px" }}>
                  Meeting and MoM
                </button>
              </li>
              <li className="sched-breadcrumb-separator"style={{ margin: "0 4px" }}>/</li>
              <li className="breadcrumb-item active d-flex align-items-center">
                <span className="sched-breadcrumb-current"style={{ marginLeft: "2px" }}>Schedule Meeting</span>
              </li>
            </ol>
          </nav>
          <form onSubmit={handleSubmit}>
                ...
                <div className="row g-4">
                  <div className="col-md-6">
                    ...
                    <div ref={meetingTypeRef} className="sched-dropdown-wrap">
                      <button type="button" className="sched-dropdown-btn"
                      ...
                      {meetingTypeOpen && (
                        <div className="sched-dropdown-menu">
                          {meetingTypeOptions.map((opt) => {
                            ...
                            return (
                              <div key={opt} className={`sched-dropdown-item${ active ? " sched-dropdown-item-active" : ""}`}
                                onClick={() => {setFormData((prev) => ({...prev,meetingType: opt,
                              }));setMeetingTypeOpen(false);
                                }}>{opt}
                              </div>
                            );
                  ...
                  <div className="col-md-6">
                    ...
                    <input type="text" name="meetingTitle" value={formData.meetingTitle}
                      onChange={handleInputChange} className="form-control sched-input" placeholder="Enter meeting title..."
                      required/>
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-semibold d-flex align-items-center gap-2">
                    ...
                    <div className="input-group mb-3 sched-search-group">
                      ...
                      <input type="text" className="form-control border-start-0 ps-0 sched-input"
                            ...
                            {filteredEmployees.length === 0 ? (
                              ...
                              filteredEmployees.map((emp) => {
                                const active =
                                  ...
                                  emp.employeeId;
                                return (
                                  <div key={emp.employeeId}  className={`sched-dropdown-item${
                                      active? " sched-dropdown-item-active" : ""
                                    }`}
                                    onClick={() => { handleOneOnOneChange(emp.employeeId);
                                      setOneOnOneOpen(false);
                                    }}
                                  >
                                  ...
                                  <tr key={emp.employeeId}
                                    ...
                                    <td className="text-start">
                                      <div className="form-check">
                                        <input className="form-check-input sched-check"
                                          type="checkbox" checked={formData.participantEmployeeIds.includes(
                                            emp.employeeId)}
                    ...
                    <div className="row">
                      <div className="col-lg-6 mb-3 mb-lg-0">
                        ...
                        <div ref={calendarRef} className="sched-date-wrap">
                          <input type="text" readOnly value={formatDisplayDate(formData.meetingDate)}
                          ...
                          <button  type="button" onClick={() => {ensureCalendarMonthYear();
                              setCalendarOpen((o) => !o);
                            }}disabled={loading} className="sched-date-icon-btn" >

                            <svg className="sched-calendar-svg"
                              width="18" height="18"viewBox="0 0 24 24" fill="none">
                          
                              <rect
                                x="4"  y="5" width="16" height="15" rx="2" ry="2"
                          ...
                          {calendarOpen && (
                            <div className="sched-calendar">
                              <div className="sched-calendar-header">
                                <button type="button" onClick={goPrevMonth} className="sched-calendar-nav">
                                  <ChevronLeft size={16} />
                                </button>
                                <span className="sched-calendar-title">{monthNames[month]} {year}</span>                             
                                <button type="button"onClick={goNextMonth}className="sched-calendar-nav">
                                  <ChevronRight size={16} />
                                </button>
                              </div>
                              <div className="sched-calendar-weekdays">
                                {weekdays.map((w) => (
                                  <div key={w} className="sched-calendar-weekday">{w}</div>
                                ))}
                              </div>
                              <div className="sched-calendar-grid">
                                {cells.map((c, idx) => {
                                  ...
                                  const isToday =
                                  c.current && cellDate.getDate() === today.getDate() &&
                                cellDate.getMonth() === today.getMonth() && cellDate.getFullYear() === today.getFullYear();
                                  const isSelected =
                                    selectedDate && c.current && cellDate.getDate() === selectedDate.getDate() &&
                                    cellDate.getMonth() ===selectedDate.getMonth() && cellDate.getFullYear() ===
                                    selectedDate.getFullYear();
                                  const classes = [
                                    "sched-calendar-cell",
                                    !c.current && "sched-calendar-cell-out", isToday && "sched-calendar-cell-today",
                                    isSelected && "sched-calendar-cell-selected",
                                  ]
                                    .filter(Boolean).join(" ");
                                  return (
                                    <div key={idx} className={classes} onClick={() =>
                                     handleSelectCalendarDay(c.day, c.current)}>{c.day}
                                    </div>
                                  );
                                })}
                              </div>
                              <div className="sched-calendar-footer">
                                <button type="button" onClick={goToday} className="sched-calendar-today"> Today </button>
                              </div>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="col-lg-6">
                        ...
                        <input type="time" name="meetingTime" value={formData.meetingTime}
                          onChange={handleInputChange}className="form-control sched-input"required />
                      </div>
                    </div>
                        ...
                        {durationOpen && (
                          <div className="sched-dropdown-menu">
                            {durationOptions.map((opt) => {
                              ...
                              return (
                                <div key={opt.value}
                                  className={`sched-dropdown-item${ active ? " sched-dropdown-item-active" : "" }`}
                                  onClick={() => {
                  ...
                  <div className="col-md-6">
                    ...
                    <div className="input-group mb-3">
                      <input type="url" name="meetingLink" value={formData.meetingLink}
                        onChange={handleInputChange} className="form-control sched-input"
                  ...
                  <div className="col-md-6">
                    ...
                    <textarea name="agenda"value={formData.agenda} onChange={handleInputChange} className="form-control sched-textarea" rows={4}
            ...
            <div className="d-flex gap-3 justify-content-end mt-4">
              <button type="button" className="btn btn-secondary px-4 d-flex align-items-center gap-2 sched-cancel-btn"
                onClick={() => navigate(-1)}>
                <X size={20} /> Cancel
              </button>
