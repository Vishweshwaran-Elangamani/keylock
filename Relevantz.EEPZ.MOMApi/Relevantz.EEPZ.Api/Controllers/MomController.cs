...
namespace eepzbackend.Controllers
{
    ...
    public partial class MomController : ControllerBase
    {
            ...
            [FromBody] CreateMomDto createMomDto)
        {
            ...
            {
                _logger.LogWarning(ex, "Validation error in CreateMom for employee {EmployeeId}", employeeId);
                return BadRequest(ApiResponse<object>.ErrorResponse(ex.Message, correlationId));
            }
            catch (UnauthorizedAccessException ex)
            {
                _logger.LogWarning(ex, "Unauthorized CreateMom attempt by employee {EmployeeId}", employeeId);
                return Forbid();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error creating MOM for employee {EmployeeId}", employeeId);
                return StatusCode(500, ApiResponse<object>.ErrorResponse(
                    "An error occurred while creating the MOM",
                    correlationId));
            }
        }
            ...
            [FromBody] UpdateMomDto updateMomDto)
        {
            ...
            {
                _logger.LogWarning(ex, "Validation error in UpdateMom for employee {EmployeeId}", employeeId);
                return BadRequest(ApiResponse<object>.ErrorResponse(ex.Message, correlationId));
            }
            catch (Exception ex)
            {
            ...
            [FromQuery] DateTime? endDate = null)
        {
            ...
            {
                _logger.LogError(ex, "Error retrieving MOMs for employee {EmployeeId}", employeeId);
                return StatusCode(500, ApiResponse<object>.ErrorResponse(
                    "An error occurred while retrieving MOMs",
                    correlationId));
            }
        }
        ...
        public async Task<ActionResult<ApiResponse<MomResponseDto>>> GetMomById(int momId)
        {
            ...
            {
                _logger.LogError(ex, "Error retrieving MOM {MomId}", momId);
                return StatusCode(500, ApiResponse<object>.ErrorResponse(
                    "An error occurred while retrieving the MOM",
                    correlationId));
            }
        }
            ...
            [FromQuery, Required] string confirmationToken)
        {
            ...
            catch (UnauthorizedAccessException ex)
            {
                _logger.LogWarning(
                    ex, 
                    "Unauthorized DeleteMom attempt for MOM {MomId} by employee {EmployeeId}",
                    momId, employeeId);
                return Forbid();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error deleting MOM {MomId} by employee {EmployeeId}", momId, employeeId);
                return StatusCode(500, ApiResponse<object>.ErrorResponse(
                    "An error occurred while deleting the MOM",
                    correlationId));
            }
        }
