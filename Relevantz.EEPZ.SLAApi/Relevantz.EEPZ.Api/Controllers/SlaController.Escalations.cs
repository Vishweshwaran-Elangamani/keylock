...
namespace eepzbackend.Controllers
{
    public partial class SlaController
    {
       ...
       [HttpPost("{slaid}/escalations")]
public async Task<IActionResult> SubmitEscalation(
    int slaid,
    [FromQuery] string? level,
    [FromBody] SubmitSlaEscalationRequest request)
{
    _logger.LogInformation("START SubmitEscalation. SLA: {Slaid}, Level: {Level}, UserId: {UserId}, CorrelationId: {CorrelationId}",
...
public async Task<IActionResult> EscalateToDeptHead([FromBody] SubmitSlaEscalationRequest request)
{
    ...
    return Ok(new ApiResponse<EscalationResponse>
    {
        ...
        {
            _logger.LogInformation("START GetSlaEscalations. SLA: {Slaid}, CorrelationId: {CorrelationId}", slaid, CorrelationId);

            var data = await _slaService.GetSlaEscalations(slaid);

            _logger.LogInformation("SUCCESS GetSlaEscalations. CorrelationId: {CorrelationId}", CorrelationId);
            _logger.LogInformation("END GetSlaEscalations. CorrelationId: {CorrelationId}", CorrelationId);
        ...
        {
            _logger.LogInformation("START ResolveEscalation. EscalationId: {EscalationId}, UserId: {UserId}, CorrelationId: {CorrelationId}",
                request.EscalationId, UserId, CorrelationId);

            await _slaService.ResolveEscalation(request, UserId);

            _logger.LogInformation("SUCCESS ResolveEscalation. CorrelationId: {CorrelationId}", CorrelationId);
            _logger.LogInformation("END ResolveEscalation. CorrelationId: {CorrelationId}", CorrelationId);
