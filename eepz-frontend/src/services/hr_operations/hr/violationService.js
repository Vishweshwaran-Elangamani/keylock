...
//   // Resolve violation (Changed from POST to PUT)
...
const violationService = {
  ...
  getAllSlaEscalations: async () => {
    try {
      ...
      throw error.response?.data || { message: "Failed to fetch SLA escalations" };
    }
  },
  ...
  getSlaEscalationsByEmployee: async (employeeUserId) => {
    try {
      const response = await hrApi.get(`/Violation/sla-escalations/employee/${employeeUserId}`);
      return response.data;
    } catch (error) {
  ...
  getSlaEscalationById: async (escalationId) => {
    try {
      const response = await hrApi.get(`/Violation/sla-escalations/${escalationId}`);
      return response.data;
    } catch (error) {
      console.error("Error fetching SLA escalation details:", error);
      throw error.response?.data || { message: "Failed to fetch escalation details" };
    }
  },
  ...
  getCombinedData: async () => {
    try {
      ...
      throw error.response?.data || { message: "Failed to fetch combined data" };
    }
  },
