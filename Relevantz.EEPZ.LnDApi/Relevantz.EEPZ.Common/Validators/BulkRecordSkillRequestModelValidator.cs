...
{
    public class BulkRecordSkillRequestModelValidator : AbstractValidator<BulkRecordSkillRequestModel>
    {
        public BulkRecordSkillRequestModelValidator()
        {
            ...
            RuleFor(x => x.Skills)
                .NotEmpty().WithMessage("Skills list cannot be empty");

            RuleForEach(x => x.Skills)
                .ChildRules(skill =>
                {
                    skill.RuleFor(x => x.SkillId)
                        .GreaterThan(0).WithMessage("SkillId must be greater than 0");

                    skill.RuleFor(x => x.Rating)
