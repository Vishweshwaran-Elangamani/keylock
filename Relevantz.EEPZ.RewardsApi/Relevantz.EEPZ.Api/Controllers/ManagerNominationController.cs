    ...
    public class ManagerNominationController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRewardTypes()
        {
            try
            {
                ...
                var rewardTypes = await _context
                    .Rewardtypes.Where(rt =>
                        rt.IsActive == true && rt.IsVisibleForManagerNomination == true
                    )
                    .Select(rt => new
                    {
                        rt.RewardTypeId,
                ...
                _logger.LogInformation(
                    $"[GET_REWARD_TYPES] Found {rewardTypes.Count} visible reward types"
                );
        ...
        public async Task<IActionResult> GetOpportunities()
        {
            try
            {
                ...
                var opportunities = await _context
                    .Recognitiondetails.Where(o =>
                        o.Status == "Active" && o.Deadline >= DateOnly.FromDateTime(DateTime.Today)
                    )
                    .ToListAsync();
                ...
                {
                    var rewardType = await _context.Rewardtypes.FirstOrDefaultAsync(rt =>
                        rt.RewardTypeId == o.RewardTypeId
                    );
        ...
        public async Task<IActionResult> GetOpportunitiesByRewardType(int rewardTypeId)
        {
            try
            {
                ...
                var validationRewardType = await _context.Rewardtypes.FirstOrDefaultAsync(rt =>
        ...
        public async Task<IActionResult> GetNominationParameters(int rewardTypeId)
        {
            try
            {
                ...
                var parameters = await _context
                    .Nominationparameters.Where(p => p.RewardTypeId == rewardTypeId)
                ...
                _logger.LogInformation(
                    $"[GET_PARAMETERS] Found {parameters.Count} parameters for Reward Type {rewardTypeId}"
                );
        ...
        public async Task<IActionResult> GetTeamMembers(int managerId)
        {
            try
            {
                ...
                _logger.LogInformation(
                    $"[GET_TEAM_MEMBERS] User Role from token: {userRole ?? "Not found"}"
                );

                var managerProjectsAsL1 = await _context
                    .Projects.Where(p =>
                        p.ResourceOwnerEmployeeId == managerId
                ...
                var resourceOwnerIds = await _context
                    .Projects.Where(p => p.ResourceOwnerEmployeeId != null)
                    .Select(p => p.ResourceOwnerEmployeeId.Value)
                    .Distinct()
                    .ToListAsync();

                var l1ApproverIds = await _context
                    .Projects.Where(p => p.L1approverEmployeeId != null)
                    .Select(p => p.L1approverEmployeeId.Value)
                    .Distinct()
                    .ToListAsync();

                var l2ApproverIds = await _context
                    .Projects.Where(p => p.L2approverEmployeeId != null)
                ...
                var teamMembers = await _context
                    .Projectemployees.Where(pe => managerProjectsAsL1.Contains(pe.ProjectId))
        ...
        public async Task<IActionResult> SubmitNomination([FromBody] NominationSubmitDto dto)
        {
            try
            {
                ...
                if (!isL1Manager)
                {
                    ...
                    return StatusCode(
                        403,
                        new { success = false, message = "Only L1 managers can submit nominations" }
                    );
                }
                ...
                var nomineeInManagerProjects = await _context.Projectemployees.AnyAsync(pe =>
                ...
                var existingNomination = await _context
                    .Recognitionstatuses.Include(n => n.Opportunity)
                    .FirstOrDefaultAsync(n =>
                        n.NomineeEmployeeId == dto.NomineeEmployeeId
                ...
                var defaultOpportunity = await _context.Recognitiondetails.FirstOrDefaultAsync(o =>
                    o.RewardTypeId == dto.RewardTypeId && o.OpportunityName.Contains("Manager")
                );
                ...
                if (dto.ParameterValues != null && dto.ParameterValues.Any())
                {
                    ...
                    foreach (var param in uniqueParams)
                    {
                        ...
                        var parameterExists = await _context.Nominationparameters.AnyAsync(np =>
                            np.ParameterId == param.ParameterId
                        ...
                        var existingParamValue =
                            await _context.Nominationparametervalues.FirstOrDefaultAsync(npv =>
                                npv.NominationId == recognitionstatus.NominationId
            ...
            {
                _logger.LogError(
                    $"[SUBMIT_NOMINATION] Error: {ex.Message}\n{ex.InnerException?.Message}"
                );
                return StatusCode(
                    500,
        ...
        public async Task<IActionResult> GetEmployeeNominations(int employeeId)
        {
            try
            {
                ...
                var nominations = await _context
                    .Recognitionstatuses.Include(n => n.Opportunity)
                        .ThenInclude(o => o.RewardType)
                    .Where(n =>
                        n.NomineeEmployeeId == employeeId && n.NominationType == "ManagerNomination"
                    )
                    .Select(n => new
                    {
                        n.NominationId,
                ...
                _logger.LogInformation(
                    $"[GET_EMPLOYEE_NOMINATIONS] Found {nominations.Count} nominations for Employee {employeeId}"
                );
        ...
        public async Task<IActionResult> GetMyNominations(int managerId)
        {
            try
            {
                ...
                var nominations = await _context
                    .Recognitionstatuses.Where(n => n.NominatedByEmployeeId == managerId)
                    .ToListAsync();
                ...
                {
                    var opportunity = await _context.Recognitiondetails.FirstOrDefaultAsync(o =>
                        o.OpportunityId == n.OpportunityId
                    );

                    var rewardType =
                        opportunity != null
                            ? await _context.Rewardtypes.FirstOrDefaultAsync(rt =>
                                rt.RewardTypeId == opportunity.RewardTypeId
                            )
                            : null;

                    var nomineeEmployee = await _context.Employees.FirstOrDefaultAsync(e =>
                        e.EmployeeId == n.NomineeEmployeeId
                    );

                    var userProfile =
                        nomineeEmployee != null
                            ? await _context.Userprofiles.FirstOrDefaultAsync(up =>
                                up.EmployeeId == nomineeEmployee.EmployeeId
                            )
                            : null;

                    var dept = await _context
                        .Employeedetailsmasters.Where(edm => edm.EmployeeId == n.NomineeEmployeeId)
                        .FirstOrDefaultAsync();

                    var department =
                        dept != null
                            ? await _context.Departments.FirstOrDefaultAsync(d =>
        ...
        public async Task<IActionResult> GetNominationDetails(int nominationId)
        {
            try
            {
                ...
                var nomination = await _context.Recognitionstatuses.FirstOrDefaultAsync(n =>
                ...
                {
                    _logger.LogWarning(
                        $"[GET_NOMINATION_DETAILS] Nomination {nominationId} not found"
                    );
                    return NotFound(new { success = false, message = "Nomination not found" });
                }

                var opportunity = await _context.Recognitiondetails.FirstOrDefaultAsync(o =>
                    o.OpportunityId == nomination.OpportunityId
                );
                ...
                var nomineeEmployee = await _context.Employees.FirstOrDefaultAsync(e =>
                    e.EmployeeId == nomination.NomineeEmployeeId
                );
                ...
                var nominatorEmployee = await _context.Employees.FirstOrDefaultAsync(e =>
                    e.EmployeeId == nomination.NominatedByEmployeeId
                );

                var nominatorProfile =
                    nominatorEmployee != null
                        ? await _context.Userprofiles.FirstOrDefaultAsync(up =>
                            up.EmployeeId == nominatorEmployee.EmployeeId
                        )
                        : null;

                var parameterValues = await _context
                    .Nominationparametervalues.Where(pv => pv.NominationId == nominationId)
                    .ToListAsync();

                var parameterResults = new List<object>();
                foreach (var pv in parameterValues)
                {
                    var parameter = await _context.Nominationparameters.FirstOrDefaultAsync(p =>
