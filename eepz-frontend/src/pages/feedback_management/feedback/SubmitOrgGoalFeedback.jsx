...
export default function SubmitOrgGoalFeedback() {
  ...
  useEffect(() => {
    const fetchObjectives = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5333/api/orgwideobjectives');
        ...
        setError('Error loading objectives: ' + (err?.message || 'Unknown error'));
      } finally {
  ...
  const handleObjectiveChange = (e) => {
    ...
    const selectedObjective = objectives.find(obj => obj.objectiveId === selectedObjectiveId);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      console.log('📤 Sending Org Objective Feedback:', JSON.stringify(payload, null, 2));

      const response = await fetch('http://localhost:5333/api/orggoalfeedback/create', {
        method: 'POST',
        headers: {
      ...
      if (data.isSuccess || response.ok) {
        setSuccessMsg('Organization objective feedback submitted successfully.');
        setForm({
          objectiveId: null,
          objectiveTitle: '',
          rating: 4,
  ...
  const selectedObjective = objectives.find(obj => obj.objectiveId === form.objectiveId);
    ...
    <div className="container-fluid py-3" style={{ maxWidth: '900px' }}>
      ...
      >
        <div className="card-body">
          {objectivesLoading ? (
            <div className="d-flex align-items-center justify-content-center gap-2" style={{ minHeight: '300px' }}>
            ...
            <div className="alert alert-warning" style={{ borderRadius: 'var(--radius-md)' }}>
              <p className="mb-0 small">No organization objectives available at the moment.</p>
            </div>
          ) : (
            <form onSubmit={submit} className="row g-4">
              ...
              <div className="col-12">
                <label className="form-label small fw-bold">
                  Select Organization Objective <span className="text-danger">*</span>
                </label>
                <select
              ...
              <div className="col-md-6">
                ...
                <div className="d-flex align-items-center gap-3">
                  <input
                    ...
                    style={{ flex: 1 }}
                  />
                  <span className="badge bg-primary px-3 py-2" style={{ minWidth: '60px', textAlign: 'center' }}>
                    {form.rating} ⭐
                  </span>
                </div>
              </div>
              ...
              <div className="col-md-6">
                ...
                <div className="form-check mt-2">
                  ...
                  />
                  <label className="form-check-label small" htmlFor="anonymousCheckbox">
                    Submit anonymously
                  </label>
                </div>
              </div>
              ...
              <div className="col-12">
                ...
                <textarea
                  ...
                  onChange={(e) => setForm({ ...form, feedbackComments: e.target.value })}
